...
async function routes(fastify, options) {
  ...
  fastify.post('/approval', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    try {
      const telegram = require('../services/telegram');
      // Smart routing: use inline buttons for actionable approval types
      const _approvalActions = ['bonus_created', 'staff_created', 'purchase_created'];
      const _typeAction = type + '_' + action;
      if (_approvalActions.includes(_typeAction) && entityId && telegram.sendApprovalRequest) {
        const _atMap = { bonus_created: 'bonus', staff_created: 'staff', purchase_created: 'purchase' };
        await telegram.sendApprovalRequest(toUserId, `🔔 *${title}*\n\n${details || ''}`, {
          type: _atMap[_typeAction] || type,
          id: entityId
        });
      } else {
