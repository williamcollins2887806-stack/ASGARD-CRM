...
async function routes(fastify, options) {
  ...
  // ═══════════════════════════════════════════════════════════════
  // POST /from-email — Создать заявку из email
  // ═══════════════════════════════════════════════════════════════
  fastify.post('/from-email', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const { subject, body, from } = request.body || {};
    if (!subject && !body) return reply.code(400).send({ error: 'Укажите тему или тело письма' });
    return { success: true, parsed: { subject, from, extracted_name: subject || 'Из письма', status: 'draft' } };
  });
  ...
  // ═══════════════════════════════════════════════════════════════
  // POST /:id/analyze — Анализ заявки
  // ═══════════════════════════════════════════════════════════════
  fastify.post('/:id/analyze', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const id = parseInt(request.params.id);
    const { rows } = await db.query('SELECT * FROM permit_applications WHERE id = $1', [id]);
    if (!rows[0]) return reply.code(404).send({ error: 'Заявка не найдена' });
    return { success: true, analysis: { status: rows[0].status, recommendation: 'Рекомендуется проверить документы', risk_level: 'low' } };
  });

  // ═══════════════════════════════════════════════════════════════
  // POST /:id/request-docs — Запрос документов
  // ═══════════════════════════════════════════════════════════════
  fastify.post('/:id/request-docs', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const id = parseInt(request.params.id);
    const { message } = request.body || {};
    const { rows } = await db.query('SELECT * FROM permit_applications WHERE id = $1', [id]);
    if (!rows[0]) return reply.code(404).send({ error: 'Заявка не найдена' });
    await db.query('UPDATE permit_applications SET status = $1, updated_at = NOW() WHERE id = $2', ['docs_requested', id]);
    return { success: true, message: 'Запрос документов отправлен' };
  });

  // ═══════════════════════════════════════════════════════════════
  // POST /:id/accept — Принять заявку
  // ═══════════════════════════════════════════════════════════════
  fastify.post('/:id/accept', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const id = parseInt(request.params.id);
    const user = request.user;
    if (!['ADMIN', 'HR_MANAGER', 'DIRECTOR_GEN', 'CHIEF_ENGINEER'].includes(user.role)) {
      return reply.code(403).send({ error: 'Недостаточно прав' });
    }
    const { rows } = await db.query('SELECT * FROM permit_applications WHERE id = $1', [id]);
    if (!rows[0]) return reply.code(404).send({ error: 'Заявка не найдена' });
    await db.query('UPDATE permit_applications SET status = $1, approved_by = $2, updated_at = NOW() WHERE id = $3', ['accepted', user.id, id]);
    return { success: true, item: { ...rows[0], status: 'accepted' } };
  });

  // ═══════════════════════════════════════════════════════════════
  // POST /:id/reject — Отклонить заявку
  // ═══════════════════════════════════════════════════════════════
  fastify.post('/:id/reject', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    const { rows } = await db.query('SELECT * FROM permit_applications WHERE id = $1', [id]);
    if (!rows[0]) return reply.code(404).send({ error: 'Заявка не найдена' });
    await db.query('UPDATE permit_applications SET status = $1, rejection_reason = $2, updated_at = NOW() WHERE id = $3', ['rejected', reason || null, id]);
    return { success: true, item: { ...rows[0], status: 'rejected' } };
  });

  // ═══════════════════════════════════════════════════════════════
  // POST /:id/fast-track — Ускоренное оформление
  // ═══════════════════════════════════════════════════════════════
  fastify.post('/:id/fast-track', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    const id = parseInt(request.params.id);
    const { rows } = await db.query('SELECT * FROM permit_applications WHERE id = $1', [id]);
    if (!rows[0]) return reply.code(404).send({ error: 'Заявка не найдена' });
    await db.query('UPDATE permit_applications SET status = $1, is_urgent = true, updated_at = NOW() WHERE id = $2', ['accepted', id]);
    return { success: true, item: { ...rows[0], status: 'accepted', is_urgent: true } };
  });
