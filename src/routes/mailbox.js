...
const MAILBOX_ROLES = ['ADMIN', 'DIRECTOR_GEN', 'DIRECTOR_COMM', 'DIRECTOR_DEV', 'HEAD_TO', 'HEAD_PM', 'PM', 'TO'];
...
async function routes(fastify, options) {
  ...
  fastify.post('/send', {
    ...
    try {
      // Get account info for From address — prefer personal account
      ...
      // Try personal email account first
      const personalAccRes = await db.query('SELECT * FROM user_email_accounts WHERE user_id = $1 AND is_active = true LIMIT 1', [user.id]);
      if (personalAccRes.rows.length > 0) {
        const pAcc = personalAccRes.rows[0];
        fromEmail = `"${pAcc.display_name || user.name || fromName}" <${pAcc.email_address}>`;
        fromName = pAcc.display_name || user.name || fromName;
      ...
      // Use personal SMTP if available
      let transport;
      if (usePersonalAccount && personalAccRes.rows.length > 0) {
        const pAcc = personalAccRes.rows[0];
        const imapSvc = require('../services/imap');
        const nodemailerMod = require('nodemailer');
        transport = nodemailerMod.createTransport({
          host: pAcc.smtp_host || 'smtp.yandex.ru',
          port: pAcc.smtp_port || 465,
          secure: pAcc.smtp_tls !== false,
          auth: { user: pAcc.smtp_user, pass: imapSvc.decrypt(pAcc.smtp_pass_encrypted) },
          connectionTimeout: 10000, greetingTimeout: 10000, socketTimeout: 15000
        });
      } else {
        transport = await getSmtpTransporter(account_id);
      }
      ...
      try {
        ...
        const corrRes = await correspondenceService.createCorrespondence(db, {
          direction: 'outgoing',
          date: new Date(),
          doc_type: 'letter',
          subject,
          ...
          work_id: request.body.work_id || null,
          status: 'sent',
          created_by: user.id
        }, {
          userId: user.id
        });
        corrNumber = corrRes.item?.number || null;
        corrId = corrRes.item?.id || null;
      } catch (corrErr) {
  ...
  fastify.post('/accounts', {
    ...
    try {
      result = await db.query(`
        ...
        RETURNING id
      `, [
        b.name, b.email_address || b.email, b.account_type || 'primary',
      ...
      // Handle unique constraint violation on email_address
      if (err.code === '23505' && err.constraint && err.constraint.includes('email_address')) {
        const emailAddr = b.email_address || b.email;
        const existing = await db.query('SELECT id FROM email_accounts WHERE email_address = $1', [emailAddr]);
        if (existing.rows.length) {
          return { success: true, id: existing.rows[0].id, note: 'already_exists' };
        }
      }
      throw err;
    }
  ...
  fastify.put('/templates/:id', {
    ...
    await db.query(`
      ...
      WHERE id = $10
    `, [
      b.name, b.category, b.subject_template || b.subject, b.body_template || b.body_html,
  ...
  fastify.post('/classification-rules', {
    ...
    const result = await db.query(`
      ...
      RETURNING id
    `, [
      b.rule_type || b.match_field || 'keyword_subject', b.pattern, b.match_mode || 'contains',
      b.classification || b.category || 'internal', b.confidence || 80, b.priority || 0,
  ...
  fastify.get('/next-outgoing-number', {
    ...
    try {
      const preview = await correspondenceService.getNextOutgoingNumberPreview(db, {
