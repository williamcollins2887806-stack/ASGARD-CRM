...
async function routes(fastify, options) {
  ...
  async function loadUserPermissions(userId, role) {
    ...
    // ADMIN получает всё
    if (role === 'ADMIN') {
      const { rows: mods } = await db.query('SELECT key as module_key FROM modules WHERE is_active = true');
      for (const m of mods) {
    ...
    if (ROLE_INHERIT[role]) rolesToCheck.push(ROLE_INHERIT[role]);

    const { rows: presetRows } = await db.query(
      `SELECT module_key, can_read, can_write, can_delete
       FROM role_presets
       WHERE role = ANY($1)
       ORDER BY can_write DESC, can_read DESC`,
      [rolesToCheck]
    );

    for (const p of presetRows) {
      if (!permissions[p.module_key]) {
        permissions[p.module_key] = { read: p.can_read, write: p.can_write, delete: p.can_delete };
      }
    }
  ...
  fastify.get('/me', {
    ...
    const result = await db.query(
      'SELECT id, login, name, patronymic, email, role, created_at, last_login_at, pin_hash FROM users WHERE id = $1',
      [request.user.id]
    );
    ...
    const has_pin = !!user.pin_hash; delete user.pin_hash; return { user: { ...user, has_pin, permissions, menu_settings } };
  });
