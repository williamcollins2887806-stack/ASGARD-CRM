...
📌 Блок "current_costs" — текущие расходы:
  ...
  - Спецодежда: 5 000₽/чел (зимняя/летняя роба, жилет сигнальный)
  - Страхование: 2 000₽/чел
  - Пропуска и инструктаж: 3 000₽/чел
  НА БРИГАДУ:
  - Амортизация оборудования: 0.5%/день от стоимости × work_days (если своё оборудование с наличии на складе)
  - Расходники: насадки, щётки, прокладки, шланги, ленты — 30-80 тыс₽ на проект
  ⚠️ НЕ ЗАБЫВАЙ: СИЗ + спецодежда = 20 000₽/чел МИНИМУМ. Это не "текущие мелочи", это КРУПНАЯ статья.
...
📌 Блок "transport" — логистика оборудования:
  - Доставка оборудования со склада на объект: Газель (малый объём) 30-50 тыс₽ или
    КамАЗ (большой объём) 80-150 тыс₽. Зависит от расстояния и объёма.
    Обратно — тоже! Итого × 2.
...
📌 Оборудование (ОБЯЗАТЕЛЬНО УЧТИ):
  - Посмотри данные склада — если оборудование ЕСТЬ → считай амортизацию в current_costs
  - Если НЕТ на складе → добавь в purchases_needed:
    - Гидромех (АВД): насос + шланги + насадки ~ аренда 100-200 тыс₽/месяц или покупка 500-1500 тыс₽
    - Химчистка: ёмкости + насосы + фитинги ~ 50-150 тыс₽
    - HVAC: инструмент + подъёмники ~ 80-200 тыс₽
    - Изоляция: леса/подмости ~ 50-100 тыс₽
  - Драгеры (газоанализаторы): если газоопасные работы — ОБЯЗАТЕЛЬНО. Аренда ~5000₽/день или покупка ~80 тыс₽/шт
  - Средства связи: рации ~3000₽/шт × (мастера + ИТР)
...

{
  ...
  "calculation": {
    ...
    "current_costs": [
      {"description": "СИЗ комплект (15000₽×кол-во): каска, ботинки, комбинезон, перчатки, очки, респиратор, привязь", "amount": <15000 × crew>},
      {"description": "Спецодежда (5000₽×кол-во): зимняя/летняя роба, жилет сигнальный", "amount": <5000 × crew>},
      {"description": "Страхование (2000₽×кол-во)", "amount": <2000 × crew>},
      {"description": "Пропуска и инструктаж (3000₽×кол-во)", "amount": <3000 × crew>},
      {"description": "Амортизация оборудования (0.5%/день от стоимости × work_days)", "amount": <расчёт если наше оборудование>},
      {"description": "Расходники: насадки, щётки, прокладки, шланги", "amount": <30000-80000 в зависимости от объёма>}
    ],
    "travel": [
    ...
    "transport": [
      {"description": "Доставка оборудования (Газель/КамАЗ туда-обратно)", "count": 1, "price": <цена>, "total": <итого>},
