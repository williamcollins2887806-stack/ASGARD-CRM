...
📌 Блок "current_costs" — текущие расходы (НА КАЖДОГО работника):
  - СИЗ: минимум 15 000₽/чел (комбинезон, каска, ботинки, перчатки, очки, респиратор, страховочная привязь)
  - Страхование: 2 000₽/чел
  - Пропуска и инструктаж: 3 000₽/чел
  - Медосмотр предрейсовый: 500₽/чел (если вахта)

📌 Блок "travel" — командировочные (рассчитывай НА КАЖДОГО × ВСЕ ДНИ):
  - Суточные: 1 000₽/чел/день × total_days (work_days + road_days*2 + моб/демоб)
  - Пайковые: 1 000₽/чел/день × total_days
  - Проживание: 1 500₽/чел/ночь МАКСИМУМ (арендуем квартиры, не гостиницы — дешевле). На весь period.
  - Билеты:
...
📌 Оборудование:
  - Если на складе ЕСТЬ нужное оборудование → амортизация 0.5%/день от стоимости
  - Если на складе НЕТ → добавь в purchases_needed (аренда или покупка)
  - Стандартный комплект для гидромеха: АВД насос, шланги, насадки ~ 50-100 тыс₽ аренда/месяц
...

{
  ...
  "calculation": {
    ...
    "current_costs": [
      {"description": "СИЗ (15000₽×кол-во)", "amount": <15000 × crew>},
      {"description": "Страхование (2000₽×кол-во)", "amount": <2000 × crew>},
      {"description": "Пропуска и инструктаж (3000₽×кол-во)", "amount": <3000 × crew>}
    ],
    "travel": [
      {"description": "Суточные (1000₽/чел/день × все дни)", "count": <crew>, "price": <total_days × 1000>, "total": <итого>},
      {"description": "Пайковые (1000₽/чел/день × все дни)", "count": <crew>, "price": <total_days × 1000>, "total": <итого>},
      {"description": "Проживание (1500₽/чел/ночь, аренда квартиры)", "count": <crew>, "price": <work_days × 1500>, "total": <итого>},
