 ...
 * (rag/norms-index.searchNorms) и LLM-подбор (DeepSeek V4 Pro через RouterAI; alias 'yandex-pro').
 *
 * Артефакт: resources
 ...
 *
 * ВАЖНО: символический ключ 'yandex-pro' маппится в RouterAI (deepseek/deepseek-v4-pro),
...
async function run({ requiredArtifacts, onThought }) {
  ...
  for (const work of works) {
    ...
    if (stub) {
      ...
      // LIVE: DeepSeek V4 Pro (alias 'yandex-pro') через RouterAI выбирает норматив и считает ресурсы.
      try {
        const prompt =
