...
═══ РЕЖИМ СМЕН ПО УМОЛЧАНИЮ — 24/7 ═══
Политика Асгард Сервис: ВСЕ работы планируем в режиме 24/7 (2 смены × 12 часов).
ВСЕГДА ставь shifts=2, кроме случаев когда в ТЗ ЯВНО написано:
  - «работы только в дневное время»
  - «доступ к объекту с 08:00 до 18:00» / «5-дневная неделя»
  - режимный объект без ночного допуска
  - запрет ночных смен (жилая зона, регламент клиента)
Если ничего из этого НЕТ — shifts=2. Не «уточняй у клиента» дефолтный режим.

Правила:
- Всегда 1 ИТР (руководитель проекта на объекте).
- 1 мастер НА КАЖДУЮ смену → при shifts=2 нужно 2 мастера.
- Рабочих/наблюдающих — по объёму и методу работ × количество смен.
...
async function run({ requiredArtifacts, onThought }) {
  ...
  // aiCompleteJson: temperature=0, AI-кэш V207 → детерминированность 5/5 для crew_composer.
  // Раньше был свой retry-loop БЕЗ кэша → бригада на одном ТЗ варьировала 15↔17 чел.
  const { aiCompleteJson } = require('./_util');
  let crew = await aiCompleteJson(aiProvider, {
    ...
    onThought, agentName: 'crew_composer',
    fallback: () => buildStubCrew(freeWorkers)
  });
  if (crew && crew._stub) crew = buildStubCrew(freeWorkers);
