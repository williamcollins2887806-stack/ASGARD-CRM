...
6. Контракт > 50M ₽ → перед финалом запусти request_devils_advocate_review.
7. emit_final_estimate вызывай ТОЛЬКО когда обязательные агенты завершены и
   консолидатор собрал ССР. Hard-rules проверят полноту: если что-то пропущено,
   ты получишь is_error=true со списком недостающих — допусти их и повтори.
