 ...
 * stubEstimated — true если для stub-агента посчитаны псевдо-токены
 * из размера артефакта (cost_rub=0, но input/output_tokens > 0).
 * Сохраняется в input_extra (ai_calls, mode, stub_estimated) для прозрачности учёта.
 */
async function finishAgentRun(agentRunId, { status = 'SUCCESS', outputArtifactId = null, outputSummary = null, errorText = null, errorCode = null, inputTokens = 0, outputTokens = 0, cacheReadTokens = 0, cacheWriteTokens = 0, costRub = 0, durationMs = 0, iterations = 0, aiCalls = 0, stubEstimated = false } = {}) {
  const mode = aiCalls > 0 ? 'live' : 'stub';
  // jsonb_set: добавляет/обновляет ai_calls, mode и stub_estimated в input_extra (NULL → {})
  await db.query(
    `UPDATE mimir_agent_runs
       SET status=$2, output_artifact_id=$3, output_summary=$4, error_text=$5, error_code=$6,
           ...
           input_extra = jsonb_set(
             ...
             '{stub_estimated}', to_jsonb($16::boolean), true
           ),
           completed_at=NOW()
     WHERE id=$1`,
