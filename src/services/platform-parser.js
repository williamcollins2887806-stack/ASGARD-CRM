...
function parseZakupkiGov(email) {
  ...
  const nmck = nmckMatch ? parseNmck(nmckMatch[1]) : null;
  const deadlineMatch = combined.match(RE_DEADLINE);
  const deadline = deadlineMatch ? extractDate(deadlineMatch[1]) : null;
  ...
  const object = objectMatch ? objectMatch[1].trim().slice(0, 500) : null;
  ...
  const method = methodMatch ? (methodMatch[1] || methodMatch[0]).trim() : null;

  const urls = [...combined.matchAll(RE_URL), ...(html.match(RE_URL) || [])].map(m => typeof m === 'string' ? m : m[0]);
  const purchaseUrl = urls.find(u => u.includes('zakupki.gov.ru')) || null;

  return { purchase_number: purchaseNumber, nmck, application_deadline: deadline, customer_name: customer, customer_inn: innMatch?.[1] || null, object_description: object, purchase_method: method, purchase_url: purchaseUrl };
}


// ═══════════════════════════════════════════════════════════════════════════
// Парсер 2: roseltorg.ru (Росэлторг)
// ═══════════════════════════════════════════════════════════════════════════
function parseRoseltorg(email) {
  ...
  return {
    purchase_number: numMatch?.[1] || null,
    nmck: nmckMatch ? parseNmck(nmckMatch[1]) : null,
    application_deadline: deadlineMatch ? extractDate(deadlineMatch[1]) : null,
    customer_name: orgMatch?.[1]?.trim() || null,
    customer_inn: (combined.match(RE_INN) || [])[1] || null,
    object_description: null,
    purchase_method: typeMatch?.[1]?.trim() || null,
...
// ═══════════════════════════════════════════════════════════════════════════
// Парсер 3: b2b-center.ru (B2B-Center)
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// Парсер 4: sberbank-ast.ru (Сбербанк-АСТ)
// ═══════════════════════════════════════════════════════════════════════════
function parseSberbankAst(email) {
  ...
  return {
    ...
    customer_inn: (combined.match(RE_INN) || [])[1] || null,
    object_description: objMatch?.[1]?.trim() || null,
    purchase_method: null,
...
// ═══════════════════════════════════════════════════════════════════════════
// Парсер 5: rts-tender.ru (РТС-Тендер)
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// Парсер 6: Универсальный (regex + AI fallback)
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// Определение площадки по email domain
// ═══════════════════════════════════════════════════════════════════════════
...
// AI-обогащение
// ═══════════════════════════════════════════════════════════════════════════
const AI_SYSTEM_PROMPT = `Ты — аналитик тендерного отдела ООО "Асгард Сервис".
Специализация компании: трубопроводы, HVAC, нефтегаз, монтаж, НПЗ, ГПЗ, МЛСП.

Проанализируй уведомление с тендерной площадки.

ФОРМАТ ОТВЕТА (строго JSON):
{
  "purchase_number": "номер или null",
  "customer_name": "заказчик",
  "customer_inn": "ИНН или null",
  "object_description": "предмет закупки (1-2 предложения)",
  "nmck": число или null,
  "application_deadline": "YYYY-MM-DD HH:MM или null",
  "purchase_method": "Аукцион/Конкурс/Запрос котировок/...",
  "relevance_score": 0-100,
  "analysis": "Краткий анализ: наш ли профиль, стоит ли участвовать (2-3 предложения)",
  "keywords": ["ключевое", "слово"]
}

relevance_score:
- 80-100: Точно наш профиль (трубопроводы, HVAC, НПЗ, обслуживание, промывка)
- 50-79: Возможно наш (монтаж, обслуживание, но не core)
- 0-49: Не наш профиль`;
...
async function enrichWithAI(email, regexResult) {
  ...
  try {
    ...
    return {
      ...
      // AI может заполнить пропущенные поля
...
// Fallback relevance score (keyword-based)
...
async function parseAndSave(emailId) {
  ...
  let aiData = null;
  try {
    aiData = await enrichWithAI(email, regexData);
  } catch (_) {}

  // Мержим данные (regex приоритетнее, AI заполняет пропуски)
  const merged = {
  ...
  // Проверяем связь с pre_tender_requests
