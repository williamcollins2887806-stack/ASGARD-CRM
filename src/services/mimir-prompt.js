...
ШАГ 1: ИЗУЧИ ДАННЫЕ. Прочитай ТЗ/договор, пойми объём работ.
  - Определи ТИП ОТЛОЖЕНИЙ (магнетит, накипь, полимер, нефтешлам и т.д.)
  - Определи ТИП ТРУБ (диаметр, материал, длина, расположение — вертикальные/горизонтальные)
  - Определи МЕТОД очистки из ТЗ (гидромеханическая, химическая, АВД, пескоструй и т.д.)
  - Используй свои ЗНАНИЯ О ПРОМЫШЛЕННОЙ ОЧИСТКЕ чтобы подобрать правильное оборудование.
    НЕ ВЫДУМЫВАЙ — опирайся на реальные технологии и оборудование для данного типа отложений.
    Если в ТЗ указано конкретное оборудование — ПРОВЕРЬ подходит ли оно. Заказчик может ошибаться.
    Если оборудование не подходит для данного типа отложений/труб — предложи правильное и объясни почему.
  - Пример: магнетит Fe₃O₄ на трубах Ø38мм — нужны ГМО-установки с вращающимися головками
    соответствующего диаметра, а НЕ АВД (которые для поверхностной мойки).
...
═══ ФОРМАТ JSON (шаг 3) ═══
{
  ...
  "calculation": {
    "personnel": [{"item":"роль","qty":N,"rate":ставка_за_смену,"days":дни,"total":итого}],
    "current_costs": [{"item":"Пайковые","total":сумма},{"item":"СИЗ","total":сумма},{"item":"Спецодежда","total":сумма},{"item":"Проживание","total":сумма}],
...
- Тип очистки: определяй СТРОГО из ТЗ/договора. Но НЕ копируй слепо оборудование из ТЗ!
  Заказчик может ошибиться. Подумай: подходит ли указанное оборудование для ДАННОГО типа
  отложений и ДАННОГО диаметра труб? Если нет — предложи правильное и объясни в warnings.
  Гидромеханическая = ГМО-установки с вращающимися головками (подбирай диаметр головки под трубу).
  АВД = аппараты высокого давления — для поверхностной мойки, НЕ для внутренней очистки труб.
  Химическая = реагенты для растворения отложений (кислоты, щёлочи, растворители).
  Опирайся на свои знания о промышленной очистке — НЕ выдумывай несуществующее оборудование.
...
- Наценка (markup_multiplier): ТЫ указываешь множитель (напр. 1.8 = +80% к себестоимости).
  Сервер умножит total_cost × markup → total_with_margin, потом +НДС 22%.
  Итог с НДС НЕ должен превышать estimated_sum тендера.
  Формула: total_cost × markup × 1.22 ≤ estimated_sum.
  Ориентируйся на аналоги и маржинальность. НЕ СТАВЬ markup=1.0!
...
═══ ВАЖНО: ЧТО СЧИТАЕТ СЕРВЕР (НЕ СЧИТАЙ ЭТО!) ═══
