-- V072: Автосинк ФОТ из field_checkins в work_expenses
-- Логика: после закрытия табеля (status='closed'/'confirmed' или checkout_at IS NOT NULL)
--   → создаём/обновляем АГРЕГИРОВАННУЮ запись в work_expenses
--   → одна запись на (work_id, employee_id) с суммой всех закрытых смен
--   → category='fot', source_table='field_checkins_agg', source_key='{work_id}:{employee_id}'
-- При удалении последней смены → удаляем агрегированную запись.
-- Если расход is_finalized=TRUE (работа закрыта) — не трогаем.
...
BEGIN
  ...
  -- Агрегируем ВСЕ закрытые смены этого сотрудника на этой работе
  SELECT
  ...
  WHERE work_id = _work_id
    ...
    AND (status IN ('closed', 'confirmed') OR checkout_at IS NOT NULL);
  ...
  IF _total_amount = 0 OR _total_days = 0 THEN
    -- Нет закрытых смен → удаляем агрегированную запись (если есть и не финализирована)
  ...
  ELSIF _existing_id IS NULL THEN
    -- Создаём новую агрегированную запись
  ...
  ELSE
    -- Обновляем существующую агрегированную запись
...
DROP TRIGGER IF EXISTS trg_sync_field_checkin_expense ON field_checkins;
CREATE TRIGGER trg_sync_field_checkin_expense
AFTER INSERT OR UPDATE OR DELETE ON field_checkins
FOR EACH ROW EXECUTE FUNCTION sync_field_checkin_to_expense();

-- Backfill: запустить sync для всех существующих field_checkins (по парам work+employee)
DO $$
DECLARE
  r RECORD;
BEGIN
  ...
  LOOP
    -- Триггерим вручную через UPDATE самой последней смены (NO-OP update)
