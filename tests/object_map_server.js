...
window.AsgardObjectMap = (function() {
  ...
  async function loadSites() {
    try {
      ...
      if (!auth || !auth.token) { toast('Ошибка', 'Сессия истекла, войдите заново', 'err'); return; }
  ...
  async function openSiteDrawer(siteId) {
    ...
    try {
      const auth = await AsgardAuth.getAuth();
      if (!auth || !auth.token) { showDrawer({ title: 'Ошибка', html: '<div class="help">Сессия истекла, войдите заново</div>' }); return; }
        ...
        document.getElementById('btnSaveSite').addEventListener('click', async () => {
          ...
          if (address || customer) {
            try {
              ...
              const auth = await AsgardAuth.getAuth();
              if (!auth || !auth.token) { toast('Ошибка', 'Сессия истекла', 'err'); return; }
