...
- **VERIFIED:** `python tools/shell_guard.py --expect-version 20.28.32 --deploy-gate` → **22/22 PASS, exit 0**;
  негативный контроль (копия прод-`index.html` без `sw.js` и с ожидаемой версией `99.99.99`) → **FAIL, exit 1**;
  на копии прод-`index.html` все содержательные проверки прошли.
...
- **Факт 1 — потерь в git нет.** Коммитов с 25.08 вне `mobile-v3`: **0** (12 коммитов за период — все в `mobile-v3`).
