...
- **Статус:** **CONFIRMED-INTENDED** (авторизация заказчика найдена 14.09; применение V353 на проде — по явному подтверждению)
- **Авторизация изменения (статус «спор» снят 14.09):**
  - **13.09.2026 17:07** (транскрипт `b80f800c`, `role=user`): заказчик просит разобрать, «что придётся менять для: (а) **подъёма порога до 10 млн**, (б) удаления выбора исполнителя и жёсткой привязки к дежурному РП»;
  - **13.09.2026 20:22** (транскрипт `afbd8058`, `role=user`): «Важно: force_director больше НЕТ — **порог реальный. approval_recipients обязательны при >=10млн**»;
  - **13.09.2026 21:11** (транскрипт `0a8ac542`, `role=user`): тест-план опирается на `work_price 12200000` (≈10 млн без НДС) как на порог pending-согласования.
  - Контр-цитата «порог 5 млн без НДС» (транскрипт `6701f668`) **датирована 01.09.2026** — на 12 дней раньше; она описывает состояние до подъёма, а не отменяет его.
