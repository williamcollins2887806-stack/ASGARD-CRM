...
- **Статус:** VERIFIED — `tests/tender-approval-chain.js` **11/11 PASS** (10 исходных + T0 на чтение настройки), exit 0 (клон `asgard_crm_test`, `:3100`)
...
- **Правка по FAIL верификатора L3 (F-1, 16.09):** в тесте была **жёсткая константа** `THRESHOLD_EX_VAT = 10_000_000`,
  из-за чего «зелёный» прогон не доказывал, что бэкенд следует **настройке**: при смене `settings` тест проверял бы
  старый порог. Теперь тест читает `settings.director_tender_threshold_rub` из БД (та же настройка, что читает бэкенд,
  `src/routes/pm-duty.js:36-46`), печатает её и **отдельным кейсом T0** сверяет с приёмочным значением 10 000 000;
  граничные кейсы B2/C2 считаются от фактической настройки (`0.6×` и `1.2×`).
  Прогон: **11/11 PASS**, exit 0 (`T0 PASS configured=10000000 accepted=10000000`).
  **Негативный контроль:** порог на клоне временно выставлен в 5 000 000 → `T0 FAIL` («приёмка требует 10000000»),
  B2/C2 пересчитались на 3 000 000 / 6 000 000 → **exit 1**; значение возвращено в 10 000 000, прогон снова зелёный.
...
- **VERIFIED (после правок по FAIL независимого верификатора):**
  1. `python tools/shell_guard.py --expect-version 20.28.32 --deploy-gate` → **37/37 PASS, exit 0**
     (строка гейта: `HEAD=08cf9ee8 lv=36d21a5e ancestor=True Δ-файлов=5 деплой-путей=0`);
...
- **Факт 1 — потерь в git нет.** Коммитов с 25.08 вне `mobile-v3`: **0** (все коммиты периода — в `mobile-v3`;
  на момент записи их было 12, к 16.09 — 13; само число растёт с каждым коммитом и в утверждении несущественно).
