/**
 * Scenario 30: FULL UI AUDIT - ALL Pages, ALL Roles, ALL Buttons/Modals/Filters
 * Comprehensive test: login as each role, visit every page, check content,
 * click create buttons, verify modals, test filters, collect JS errors
 */
const SCENARIO_NAME = '30-full-ui-audit';
...
// Browser health check + auto-relaunch
  ...
  const CREATE_PAGES = [
    ...
    { route: 'all-works',      roles: ['ADMIN'] },
    { route: 'all-estimates',  roles: ['ADMIN'] },
    ...
    { route: 'employee',       roles: ['HR', 'ADMIN'] },
    { route: 'hr-requests',    roles: ['HR', 'ADMIN'] },
    { route: 'proc-requests',  roles: ['ADMIN', 'PROC'] },
    { route: 'correspondence', roles: ['ADMIN'] },
    { route: 'contracts',      roles: ['ADMIN', 'OFFICE_MANAGER'] },
    { route: 'seals',          roles: ['ADMIN'] },
    { route: 'proxies',        roles: ['ADMIN'] },
    { route: 'travel',         roles: ['ADMIN', 'HR'] },
    { route: 'pass-requests',  roles: ['ADMIN', 'PM'] },
    { route: 'tmc-requests',   roles: ['ADMIN', 'PM'] },
    { route: 'meetings',       roles: ['ADMIN', 'PM'] },
    { route: 'warehouse',      roles: ['ADMIN'] },
    { route: 'acts',           roles: ['ADMIN', 'PM'] },
    { route: 'payroll',        roles: ['ADMIN', 'BUH'] },
    { route: 'self-employed',  roles: ['ADMIN', 'BUH'] },
    { route: 'one-time-pay',   roles: ['ADMIN', 'PM'] },
    { route: 'office-expenses',roles: ['ADMIN', 'OFFICE_MANAGER'] },
    { route: 'permit-applications', roles: ['ADMIN', 'HR'] },
  ];

  try {
    // Phase 1: Test each role's menu and page access
    const testRoles = ['ADMIN', 'DIRECTOR_GEN', 'DIRECTOR_COMM', 'DIRECTOR_DEV', 'HEAD_PM', 'HEAD_TO', 'PM', 'TO', 'HR', 'HR_MANAGER', 'BUH', 'OFFICE_MANAGER', 'PROC', 'CHIEF_ENGINEER', 'WAREHOUSE'];
    ...
    for (const role of testRoles) {
      await step(`${role}: Menu items and page navigation`, async (s) => {
        ...
        try {
          ...
          const pageResults = [];
          for (const item of menuItems) { // Limit to 8 pages per role
            try {
    ...
    const DETAIL_PAGES = [
      ...
      { route: 'tkp',         role: 'ADMIN' },
      { route: 'invoices',    role: 'ADMIN' },
      { route: 'tasks',       role: 'ADMIN' },
      { route: 'employee',    role: 'HR' },
      { route: 'cash',        role: 'ADMIN' },
      { route: 'contracts',   role: 'ADMIN' },
      { route: 'correspondence', role: 'ADMIN' },
      { route: 'warehouse',   role: 'ADMIN' },
      { route: 'acts',        role: 'ADMIN' },
      { route: 'payroll',     role: 'ADMIN' },
    ];
