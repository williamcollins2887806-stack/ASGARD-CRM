...
async function routes(fastify, options) {
  ...
  fastify.get('/analytics/team', {
    ...
    const teamKpi = await db.query(`
      ...
      FROM users u
      LEFT JOIN tenders t ON (t.created_by = u.id) AND ${whereClause}
  ...
  fastify.post('/:id/calc-cost', {
    ...
    try {
      ...
      // Placeholder: will use custom prompt from user later
      return { success: true, message: 'Расчёт себестоимости будет доступен после настройки промпта' };
    } catch (err) {
