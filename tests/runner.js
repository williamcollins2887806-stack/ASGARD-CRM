 ...
 *   node tests/runner.js --scenarios  # Playwright scenarios only
 */
...
const runScenarios = args.includes('--all') || args.includes('--scenarios');

const results = { api: [], smoke: [], e2e: [], scenarios: [], summary: {} };
...
async function main() {
  ...
  // ═══ СЛОЙ 4: СЦЕНАРИИ (Playwright browser tests) ═══
  if (runScenarios) {
    console.log('\n━━━ СЛОЙ 4: СЦЕНАРИИ (Scenarios) ━━━');
    const scenDir = path.join(__dirname, 'scenarios');
    if (fs.existsSync(scenDir)) {
      const scenFiles = fs.readdirSync(scenDir).filter(f => f.endsWith('.test.js')).sort();

      // Launch Playwright browser once for all scenarios
      let browser = null;
      try {
        const { chromium } = require('playwright');
        browser = await chromium.launch({ headless: true, args: ['--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu', '--disable-extensions', '--disable-background-networking', '--disable-default-apps', '--no-first-run'] });
        console.log('  [runner] Playwright browser launched');
      } catch (e) {
        console.log('  [runner] Playwright not available: ' + e.message);
      }

      for (const file of scenFiles) {
        try {
          const suite = require(path.join(scenDir, file));

          if (suite.tests && Array.isArray(suite.tests)) {
            // New format: { name, tests: [{ name, run }] }
            // Check if browser is alive for suites that may need it
            if (browser) {
              try { const _ctx = await browser.newContext(); await _ctx.close(); } catch (_) {
                console.log('  [runner] Browser died, relaunching for suite...');
                try {
                  const { chromium } = require('playwright');
                  browser = await chromium.launch({ headless: true, args: ['--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu', '--disable-extensions', '--disable-background-networking', '--disable-default-apps', '--no-first-run'] });
                  console.log('  [runner] Browser relaunched');
                } catch (e2) { browser = null; }
              }
            }
            const r = await runTestSuite(suite);
            results.scenarios.push({ name: suite.name, results: r });
          } else if (typeof suite.run === 'function') {
            // Legacy Playwright format: { name, run(browser) }
            console.log(`\n  ▸ ${suite.name || file} (Playwright scenario)`);
            const t0 = Date.now();
            try {
              if (!browser) {
                console.log(`    ⏭️  ${suite.name || file}: SKIP — Playwright not available`);
                results.scenarios.push({ name: suite.name || file, results: [{ name: suite.name || file, status: 'SKIP', ms: 0 }] });
                continue;
              }
              ...
              try {
                ...
                try {
                  ...
                  console.log('  [runner] Failed to relaunch browser: ' + relaunchErr.message);
                  browser = null;
                  console.log(`    ⏭️  ${suite.name || file}: SKIP — Browser unavailable`);
                  results.scenarios.push({ name: suite.name || file, results: [{ name: suite.name || file, status: 'SKIP', ms: 0 }] });
                  continue;
                }
              }
              const result = await suite.run(browser, {});
              const ms = Date.now() - t0;

              if (result && result.steps && result.steps.length > 0) {
                // Scenario returned detailed step results
                const stepResults = result.steps.map(s => ({
                  name: s.name,
                  status: s.status === 'PASSED' ? 'PASS' : (s.status === 'FAILED' ? 'FAIL' : s.status),
                  ms: s.duration || 0,
                  error: s.error || undefined
                }));
                const passCount = stepResults.filter(s => s.status === 'PASS').length;
                const failCount = stepResults.filter(s => s.status === 'FAIL').length;
                console.log(`    ${failCount > 0 ? '❌' : '✅'} ${suite.name || file}: ${passCount} pass, ${failCount} fail (${ms}ms)`);
                results.scenarios.push({ name: suite.name || file, results: stepResults });
              } else {
                console.log(`    ✅ ${suite.name || file} (${ms}ms)`);
                results.scenarios.push({ name: suite.name || file, results: [{ name: suite.name || file, status: 'PASS', ms }] });
              }
            } catch (runErr) {
              const ms = Date.now() - t0;
              if (runErr.name === 'SkipError') {
                console.log(`    ⏭️  ${suite.name || file}: SKIP — ${runErr.message}`);
                results.scenarios.push({ name: suite.name || file, results: [{ name: suite.name || file, status: 'SKIP', ms }] });
              } else {
                console.log(`    ❌ ${suite.name || file}: ${runErr.message?.slice(0, 200)}`);
                results.scenarios.push({ name: suite.name || file, results: [{ name: suite.name || file, status: 'FAIL', ms, error: runErr.message?.slice(0, 300) }] });
              }
            }
          } else {
            console.log(`    ⚠️  ${file}: unknown format (no .tests or .run)`);
          }
        } catch (err) {
          console.log(`    ⚠️  Failed to load ${file}: ${err.message}`);
          results.scenarios.push({ name: file, results: [{ name: 'Load ' + file, status: 'FAIL', ms: 0, error: err.message }] });
        }
      }
      ...
      if (browser) {
        ...
        console.log('  [runner] Playwright browser closed');
      }
    }
  }
  ...
  const allTests = [
    ...
    ...results.e2e,
    ...results.scenarios.flatMap(s => s.results || [])
  ];
...
${results.smoke.map(t => `<tr>
  ...
  const scenariosSection = results.scenarios.length > 0 ? `
<h2>SCENARIOS (Playwright)</h2>
${results.scenarios.map(suite => `
<h3>${esc(suite.name)}</h3>
<table>
<tr><th>Step</th><th>Status</th><th>Time</th><th>Error</th></tr>
${(suite.results || []).map(t => `<tr>
  <td>${esc(t.name)}</td>
  <td><span class="tag-${t.status.toLowerCase()}">${t.status}</span></td>
  <td>${t.ms || 0}ms</td>
