...
module.exports = {
  ...
  tests: [
...

    {
      ...
      run: async () => {
        // Fetch a real employee ID (employees.id, NOT users.id)
      const empResp = await api('GET', '/api/data/employees?limit=1', { role: 'ADMIN' });
      const empList = empResp.data?.employees || empResp.data?.rows || empResp.data?.items || [];
      if (empList.length === 0) skip('No employees in database');
      const employeeId = empList[0].id;
    
      // Fetch a real permit type ID
      const typesResp = await api('GET', '/api/permits/types', { role: 'ADMIN' });
      const types = typesResp.data?.types || typesResp.data || [];
      if (!Array.isArray(types) || types.length === 0) skip('No permit types in database');
      const typeId = types[0].id;
    
      // Create permit with valid IDs
      const createResp = await api('POST', '/api/permits', {
        role: 'HR',
        body: {
          employee_id: employeeId,
          type_id: typeId,
          doc_number: 'E2E-BIZ-PERMIT-' + Date.now(),
      ...
      if (createResp.status === 400 || createResp.status === 403) {
        skip('Permits create not available: ' + createResp.status + ' ' + JSON.stringify(createResp.data));
      }
      assertOk(createResp, 'create permit');
      const permitId = createResp.data?.permit?.id || createResp.data?.id;
      assert(permitId, 'permit ID returned');
      ...
      // Update permit notes
      if (permitId) {
        const upd = await api('PUT', '/api/permits/' + permitId, {
          role: 'HR',
          body: { notes: 'Updated E2E permit test' }
        });
        assert([200, 404].includes(upd.status), 'update permit: ' + upd.status);
      }
    
      // Cleanup
      if (permitId) {
        await api('DELETE', '/api/permits/' + permitId, { role: 'ADMIN' });
      }
      }
    },
