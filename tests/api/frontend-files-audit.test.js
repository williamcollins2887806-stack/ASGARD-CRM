...
module.exports = {
  ...
  tests: [
    ...
    { name: '8.01 path traversal: /assets/../../etc/passwd blocked', run: async () => {
      await assertBlocked('/assets/../../etc/passwd', 'traversal etc/passwd');
    }},

    { name: '8.02 path traversal: /../../../etc/shadow blocked', run: async () => {
      await assertBlocked('/../../../etc/shadow', 'traversal etc/shadow');
    }},
    ...
    { name: '8.15 encoded traversal: /%2e%2e/%2e%2e/etc/passwd blocked', run: async () => {
      await assertBlocked('/%2e%2e/%2e%2e/etc/passwd', 'encoded traversal');
    }},
