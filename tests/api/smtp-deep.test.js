...
module.exports = {
  ...
  tests: [
    ...
    {
      name: 'SMTP-10: PM sends email (PM has mailbox access)',
      run: async () => {
        const resp = await api('POST', '/api/mailbox/send', {
          role: 'PM',
          body: { to: 'test@example.com', subject: 'PM test', body_text: 'test' }
        });
        // PM is in MAILBOX_ROLES; may get 200, 400, or 500 depending on SMTP config, but NOT 403
        assert(resp.status !== 403, 'PM has mailbox access, should not get 403, got ' + resp.status);
      }
    },
    {
