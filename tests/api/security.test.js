  ...
  tests: [
    ...
    },
    {
      ...
      run: async () => {
        ...
        // Cleanup: delete by INN and by name (to avoid unique constraint on name from previous runs)
        await api('DELETE', `/api/customers/${inn}`, { role: 'ADMIN' }).catch(() => {});
        // Also try to find and delete customer with same name
        const existing = await api('GET', '/api/data/customers?limit=100', { role: 'ADMIN' });
        const customers = existing.data?.customers || [];
        for (const c of customers) {
          if (c.name === sqli && c.inn !== inn) {
            await api('DELETE', `/api/customers/${c.inn}`, { role: 'ADMIN' }).catch(() => {});
          }
        }
