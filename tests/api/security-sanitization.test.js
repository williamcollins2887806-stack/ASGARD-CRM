...
const {
  api, assert, assertStatus, rawFetch, skip,
  BASE_URL, JWT_SECRET, getToken
} = require('../config');
...
module.exports = {
  ...
  tests: [
    ...
    {
      name: 'AUTHZ-01: HEAD_TO GET /api/data/tenders -> 403',
      run: async () => {
        const resp = await api('GET', '/api/data/tenders', { role: 'HEAD_TO' });
        assertStatus(resp, 403, 'HEAD_TO on /api/data/tenders');
      }
    },
    // D2
    {
    ...
    // D3
    {
      name: 'AUTHZ-03: CHIEF_ENGINEER GET /api/data/tenders -> 403',
      run: async () => {
        const resp = await api('GET', '/api/data/tenders', { role: 'CHIEF_ENGINEER' });
        assertStatus(resp, 403, 'CHIEF_ENGINEER on /api/data/tenders');
      }
    },
    // D4
    {
    ...
    // D8
    {
      name: 'AUTHZ-08: PROC GET /api/data/tenders -> 403',
      run: async () => {
        const resp = await api('GET', '/api/data/tenders', { role: 'PROC' });
        assertStatus(resp, 403, 'PROC on /api/data/tenders');
      }
    },
    // D9
    {
      name: 'AUTHZ-09: PROC GET /api/data/employees -> 403',
      run: async () => {
        const resp = await api('GET', '/api/data/employees', { role: 'PROC' });
        assertStatus(resp, 403, 'PROC on /api/data/employees');
      }
    },
    // D10
    {
      name: 'AUTHZ-10: PROC GET /api/data/works -> 403',
      run: async () => {
        const resp = await api('GET', '/api/data/works', { role: 'PROC' });
        assertStatus(resp, 403, 'PROC on /api/data/works');
      }
    },
    // D11
    {
    ...
    // D14
    {
      name: 'AUTHZ-14: HEAD_TO GET /api/data/employees -> 403',
      run: async () => {
        const resp = await api('GET', '/api/data/employees', { role: 'HEAD_TO' });
        assertStatus(resp, 403, 'HEAD_TO on /api/data/employees');
      }
    },
    // D15
    {
    ...
    // D18
    {
      name: 'AUTHZ-18: HR_MANAGER GET /api/data/tenders -> 403',
      run: async () => {
        const resp = await api('GET', '/api/data/tenders', { role: 'HR_MANAGER' });
        assertStatus(resp, 403, 'HR_MANAGER on /api/data/tenders');
      }
    },
    // D19
    {
      name: 'AUTHZ-19: HR_MANAGER GET /api/data/works -> 403',
      run: async () => {
        const resp = await api('GET', '/api/data/works', { role: 'HR_MANAGER' });
        assertStatus(resp, 403, 'HR_MANAGER on /api/data/works');
      }
    },
    // D20
    {
    ...
    // D21
    {
      name: 'AUTHZ-21: TO POST /api/data/works (write to read-only table) -> 403',
      run: async () => {
        const resp = await api('POST', '/api/data/works', {
          role: 'TO',
          body: { work_title: 'TO should not write works' }
        });
        assertStatus(resp, 403, 'TO write to works via data API');
      }
    },
    // D22
    {
      name: 'AUTHZ-22: TO POST /api/data/employees (no access) -> 403',
      run: async () => {
        const resp = await api('POST', '/api/data/employees', {
          role: 'TO',
          body: { fio: 'TO should not write employees' }
        });
        assertStatus(resp, 403, 'TO write to employees via data API');
      }
    },
    // D23
    {
    ...
    // D24
    {
      name: 'AUTHZ-24: HEAD_TO POST /api/data/works (no access) -> 403',
      run: async () => {
        const resp = await api('POST', '/api/data/works', {
          role: 'HEAD_TO',
          body: { work_title: 'HEAD_TO should not write works' }
        });
        assertStatus(resp, 403, 'HEAD_TO write to works via data API');
      }
    },
    // D25
    {
    ...
    // E1
    {
      ...
      run: async () => {
        ...
        assert(
          ...
          `path traversal ../../etc/passwd: expected 400/403/404, got ${resp.status}`
        );
      }
    },
    // E2
    {
      ...
      run: async () => {
        ...
        assert(
          [400, 403, 404].includes(resp.status),
          `path traversal tenders/../../../etc/passwd: expected 400/403/404, got ${resp.status}`
        );
      }
    },
    // E3
    ...
    // E4
    {
      name: 'PATH-04: table name with ../',
      run: async () => {
        const resp = await api('GET', '/api/data/../tenders', { role: 'ADMIN' });
        assert(
          [400, 403, 404].includes(resp.status),
          `path traversal ../tenders: expected 400/403/404, got ${resp.status}`
        );
      }
    },
    // E5
