 ...
 */
const { api, assert, assertOk, assertStatus, assertForbidden, rawFetch, skip, BASE_URL, getToken, getTokenSync } = require('../config');
  ...
  tests: [
    ...
    // ═══ SQL Injection attempts ═══
    {
      ...
      run: async () => {
        ...
        // C4: onRequest hook blocks `;` in URL → 400 (safe), or Fastify parses it → 200 (also safe)
        assert([200, 400, 403].includes(resp.status), `SQL injection in table: expected 200/400/403, got ${resp.status}`);
      }
    },
    {
