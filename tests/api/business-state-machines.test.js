...
const TENDER_STATUSES = ['Новый', 'В работе', 'Подготовка КП', 'Подан', 'Выигран', 'Проигран', 'Отменён',
  'Оценка', 'Торги', 'Выиграли', 'Проиграли', 'Контракт', 'Отказ', 'Архив', 'Мобилизация'];

const WORK_STATUSES = ['Активная', 'Завершена', 'Приостановлена', 'Отменена',
...
const EQUIPMENT_REQUEST_STATUSES = ['new', 'approved', 'issued', 'returned', 'rejected'];

const STAFF_REQUEST_STATUSES = ['new', 'approved', 'rejected', 'closed'];
...
const TASK_STATUSES = ['new', 'in_progress', 'done', 'cancelled', 'accepted', 'pending', 'completed'];
...
module.exports = {
  ...
  tests: [
    ...
    // #65
    {
      name: 'SM-65: PM should NOT access staff_requests (403)',
      run: async () => {
        const resp = await api('GET', '/api/data/staff_requests?limit=5', { role: 'PM' });
        if (resp.status === 404) skip('endpoint not found');
        assert(
          resp.status === 403 || resp.status === 401,
          `PM staff_requests: expected 403/401, got ${resp.status}`
        );
      }
    },
    // #66
