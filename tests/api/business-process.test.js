 ...
 *
 * 1.  Тендер → Просчёт → Работа → Акт → Счёт
 * 2.  Задача: создать → принять → выполнить
 * 3.  ТКП: создать → отправить → согласовать
 * 4.  Заявка на пропуск: создать → подать → одобрить
 * 5.  Заявка ТМЦ: создать → подать → одобрить
 * 6.  Уведомления: создать → прочитать
 * 7.  Чат: создать группу → отправить сообщение
 * 8.  Поиск: найти тендер по слову
 * 9.  Финансы: cash advance → approve
 * 10. Мультиролевой: PM создаёт, ADMIN проверяет
 */
...
// Helper: try create, skip if 500 error (schema/FK issues)
async function tryCreate(method, path, opts) {
  const resp = await api(method, path, opts);
  if (resp.status === 500) {
    const msg = JSON.stringify(resp.data || '');
    skip(`Server error on ${method} ${path}: ${msg.slice(0, 120)}`);
  }
  return resp;
}

// ═══════════════════════════════════════════════════════════════
// SCENARIO 1: Тендер → Просчёт → Работа → Счёт (полный цикл)
// ═══════════════════════════════════════════════════════════════

tests.push({ name: 'BP1: Тендер → создать', run: async () => {
  const resp = await tryCreate('POST', '/api/tenders', {
    role: 'ADMIN',
    body: {
  ...
  const tender = resp.data?.item || resp.data?.tender || resp.data;
...
tests.push({ name: 'BP1: Просчёт → создать для тендера', run: async () => {
  ...
  const resp = await tryCreate('POST', '/api/estimates', {
    role: 'ADMIN',
    body: {
      tender_id: ctx.tenderId,
      estimate_type: 'Основной',
      total_price: 500000,
      cost_total: 350000,
      margin_pct: 30
    }
  ...
  const est = resp.data?.item || resp.data?.estimate || resp.data;
...
tests.push({ name: 'BP1: Работа → создать из тендера', run: async () => {
  if (!ctx.tenderId) skip('no tender');
  const resp = await tryCreate('POST', '/api/works', {
    role: 'ADMIN',
    body: {
      tender_id: ctx.tenderId,
  ...
  const work = resp.data?.item || resp.data?.work || resp.data;
...
tests.push({ name: 'BP2: Задача → создать', run: async () => {
  if (!ctx.tenderId) skip('no tender');
  const resp = await tryCreate('POST', `/api/tasks/${ctx.tenderId}`, {
    role: 'ADMIN',
    body: {
  ...
  if (resp.status === 404) skip('tasks endpoint format differs');
  assertOk(resp, 'create task');
  const task = resp.data?.item || resp.data?.task || resp.data;
...
tests.push({ name: 'BP2: Задача → принять', run: async () => {
  ...
  const resp = await api('PUT', `/api/tasks/${ctx.taskId}/accept`, { role: 'ADMIN' });
  if (resp.status === 404) skip('accept endpoint not available');
  assertOk(resp, 'accept task');
}});

tests.push({ name: 'BP2: Задача → выполнить', run: async () => {
  if (!ctx.taskId) skip('no task');
  const resp = await api('PUT', `/api/tasks/${ctx.taskId}/complete`, { role: 'ADMIN' });
  if (resp.status === 404) skip('complete endpoint not available');
...
tests.push({ name: 'BP3: ТКП → создать', run: async () => {
  const resp = await tryCreate('POST', '/api/tkp', {
    role: 'ADMIN',
    body: {
      title: 'BP-Test ТКП',
      customer_name: 'BP Заказчик',
      total_sum: 1200000,
      services: 'Промывка теплообменника',
      deadline: '2026-04-01',
      validity_days: 30,
      content_json: JSON.stringify({ items: [{ name: 'Промывка', price: 1200000 }] })
    }
  });
...
tests.push({ name: 'BP4: Пропуск → создать', run: async () => {
  const resp = await tryCreate('POST', '/api/pass-requests', {
    role: 'ADMIN',
    body: {
...
tests.push({ name: 'BP5: ТМЦ → создать', run: async () => {
  const resp = await tryCreate('POST', '/api/tmc-requests', {
    role: 'ADMIN',
    body: {
      title: 'BP-Test ТМЦ',
      priority: 'high',
...
tests.push({ name: 'BP6: Уведомления → пометить прочитанным', run: async () => {
  ...
  if (mark.status === 404) skip('mark read endpoint not available');
...
tests.push({ name: 'BP7: Чат → создать группу', run: async () => {
  const resp = await tryCreate('POST', '/api/chat-groups', {
    role: 'ADMIN',
    body: {
  ...
  const chat = resp.data?.item || resp.data?.chat || resp.data;
...
tests.push({ name: 'BP7: Чат → отправить сообщение', run: async () => {
  ...
  const resp = await tryCreate('POST', `/api/chat-groups/${ctx.chatId}/messages`, {
    role: 'ADMIN',
...
// ═══════════════════════════════════════════════════════════════
// SCENARIO 8: Глобальный поиск
// ═══════════════════════════════════════════════════════════════

tests.push({ name: 'BP8: Поиск → API доступен', run: async () => {
  const resp = await api('GET', '/api/search?q=BP-Test', { role: 'ADMIN' });
  if (resp.status === 404) skip('search endpoint not available');
  assertOk(resp, 'search');
}});

// ═══════════════════════════════════════════════════════════════
// SCENARIO 9: Финансы — endpoints
// ═══════════════════════════════════════════════════════════════

tests.push({ name: 'BP9: Финансы → GET /api/finances', run: async () => {
  const resp = await api('GET', '/api/finances', { role: 'ADMIN' });
  if (resp.status === 404) skip('finances endpoint not available');
  assertOk(resp, 'get finances');
}});

tests.push({ name: 'BP9: Расходы → GET /api/expenses', run: async () => {
  const resp = await api('GET', '/api/expenses', { role: 'ADMIN' });
  if (resp.status === 404) skip('expenses endpoint not available');
  assertOk(resp, 'get expenses');
}});
