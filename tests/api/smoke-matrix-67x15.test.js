...
// pre-tenders: ALLOWED_ROLES = ['ADMIN','DIRECTOR_GEN','DIRECTOR_COMM','DIRECTOR_DEV','HEAD_TO','HEAD_PM','TO']
// With inheritance: HEAD_TO is explicit, HEAD_PM is explicit, HR_MANAGER inherits HR (not listed), CHIEF_ENGINEER inherits WAREHOUSE (not listed)
const PRE_TENDER_ALLOWED = ['ADMIN', 'DIRECTOR_GEN', 'DIRECTOR_COMM', 'DIRECTOR_DEV', 'HEAD_TO', 'HEAD_PM', 'TO'];
...
// permits (read): V005 role_presets give TO, PM, HR, CHIEF_ENGINEER + ADMIN + directors
...
// HEAD_TO, HR_MANAGER, CHIEF_ENGINEER now have access per ACCESS_MATRIX → 403 on all
const DATA_ALL_ROLES = ['ADMIN','DIRECTOR_GEN','DIRECTOR_COMM','DIRECTOR_DEV'];
const DATA_TABLE_ENDPOINTS = [
  { page: '/correspondence',  table: 'correspondence',     extraRoles: ['PM','TO','OFFICE_MANAGER','HEAD_TO'] },
  ...
  { page: '/hr-requests',     table: 'staff_requests',      extraRoles: ['HR','HR_MANAGER','PM','HEAD_PM'] },
  { page: '/user-requests',   table: 'staff_requests',      extraRoles: ['HR','HR_MANAGER','PM','HEAD_PM'] },
  { page: '/travel',          table: 'travel_expenses',     extraRoles: ['PM','HR','OFFICE_MANAGER','HR_MANAGER'] },
];
...
const INHERITANCE_TESTS = [
  ...
  { role: 'HEAD_PM', inheritsFrom: 'PM',  endpoint: 'GET /api/pre-tenders', shouldAccess: true,  desc: 'HEAD_PM is explicitly in ALLOWED_ROLES for pre-tenders' },
