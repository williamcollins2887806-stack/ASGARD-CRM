...
let tmcId = null;
/**
 * Retry wrapper for rawFetch - handles connection exhaustion
 * that occurs when this test runs late in the suite (after 4800+ tests).
 */
async function retryFetch(method, urlPath, opts = {}, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      return await rawFetch(method, urlPath, opts);
    } catch (e) {
      if (i === maxRetries - 1) throw e;
      console.log('  [retryFetch] ' + method + ' ' + urlPath + ' failed (attempt ' + (i + 1) + '/' + maxRetries + '): ' + e.message + ', retrying in 2s...');
...
module.exports = {
  ...
  tests: [
    ...
    // ═══════════════════════════════════════════════════════════════
    {
      ...
      run: async () => {
        const resp = await retryFetch('POST', '/api/auth/login', {
    ...
    },
    {
      ...
      run: async () => {
        const resp = await retryFetch('GET', '/api/users');
