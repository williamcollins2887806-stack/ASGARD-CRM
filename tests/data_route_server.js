...
async function dataRoutes(fastify, options) {
  ...
  const ACCESS_MATRIX = {
    ...
    TO: {
      tables: [
        'users', 'employees', 'staff', 'works', 'invoices', 'user_call_status',
    ...
    BUH: {
      tables: [
        'users', 'employees', 'staff', 'cash_requests', 'cash_expenses', 'cash_returns', 'cash_messages', 'user_call_status',
    ...
    OFFICE_MANAGER: {
      tables: [
        ...
        'travel_expenses', 'doc_sets', 'user_dashboard', 'customers', 'works'
      ],
      ops: ['read', 'create', 'update']
    },
    WAREHOUSE: {
  ...
  fastify.post('/:table', {
    ...
    try {
      ...
      // Add ON CONFLICT upsert for tables with natural keys
      let query;
      if (pk !== 'id' && keys.includes(pk)) {
        const updateCols = keys.filter(k => k !== pk).map((k, i) => k + ' = EXCLUDED.' + k).join(', ');
        query = `
          INSERT INTO ${dbTable} (${keys.join(', ')})
          VALUES (${placeholders.join(', ')})
          ON CONFLICT (${pk}) DO UPDATE SET ${updateCols || 'updated_at = NOW()'}
          RETURNING *
        `;
      } else {
        query = `
