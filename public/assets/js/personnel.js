  ...
  async function render({layout,title}){
    ...
    const html = `
      <div class="panel">
        ...
        <div id="personnel_pagination"></div>
        </div>
    ...
    if (window.AsgardPagination && rows.length > 0) {
      const tbody = document.getElementById("personnel_tbody");
      if (tbody) {
        const allRows = Array.from(tbody.querySelectorAll("tr"));

        function applyPagination(pg, ps) {
          allRows.forEach(function(r, i) {
            if (ps > 0) { r.style.display = (i >= (pg-1)*ps && i < pg*ps) ? "" : "none"; }
            else { r.style.display = ""; }
          });
          const pgEl = document.getElementById("personnel_pagination");
          if (pgEl) {
            pgEl.innerHTML = AsgardPagination.renderControls(allRows.length, pg, ps);
            AsgardPagination.attachHandlers("personnel_pagination",
              function(p) { currentPage = p; applyPagination(p, pageSize); },
              function(s) { pageSize = s; currentPage = 1; applyPagination(1, s); }
            );
          }
        }
