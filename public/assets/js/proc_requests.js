window.AsgardProcRequestsPage=(function(){
  ...
  async function render({layout,title}){
    ...
    async function load(){
      ...
      const paged_proc = window.AsgardPagination ? AsgardPagination.paginate(list, currentPage, pageSize) : list;
      tb.innerHTML = paged_proc.map(r=>{
        const w=works.find(x=>x.id===r.work_id);
        const t=w?tenders.find(x=>x.id===w.tender_id):null;
      ...
      cnt.textContent = `Показано: ${paged_proc.length} из ${list.length}`;
      if (window.AsgardPagination) {
        let pgEl = document.getElementById("proc_pagination");
        if (!pgEl) { pgEl = document.createElement("div"); pgEl.id = "proc_pagination"; tb.closest("table").after(pgEl); }
        pgEl.innerHTML = AsgardPagination.renderControls(list.length, currentPage, pageSize);
        AsgardPagination.attachHandlers("proc_pagination",
    ...
    async function openReq(id){
      ...
      $("#btnSend").addEventListener("click", async ()=>{
        ...
        await notify(req.pm_id,"Ответ закупок", `${w.customer_name||t.customer_name} — ${w.work_title||t.tender_title}`, "#/pm-works");
