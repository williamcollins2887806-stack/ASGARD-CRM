...
window.AsgardWarehouse = (function(){
  ...
  async function refreshTable(isDirector = false) {
    ...
    if (tbody) {
      const pagedEquip = window.AsgardPagination ? AsgardPagination.paginate(data.equipment, currentPage, pageSize) : data.equipment;
        tbody.innerHTML = renderEquipmentRows(pagedEquip || [], canEdit, isDirector);
      // Пагинация
      if (window.AsgardPagination) {
        let pgEl = document.getElementById("warehouse_pagination");
        if (!pgEl && tbody.closest("table")) { pgEl = document.createElement("div"); pgEl.id = "warehouse_pagination"; tbody.closest("table").after(pgEl); }
        if (pgEl) {
          pgEl.innerHTML = AsgardPagination.renderControls(data.equipment.length, currentPage, pageSize);
          AsgardPagination.attachHandlers("warehouse_pagination",
            (p) => { currentPage = p; refreshTable(); },
            (s) => { pageSize = s; currentPage = 1; refreshTable(); }
          );
        }
      }
    }
