...
window.AsgardBuhRegistryPage = (function(){
  ...
  async function render({layout, title}){
    ...
    works.forEach(w => {
      ...
      const name = w.customer_name || t?.customer_name || '';
    ...
    const body = `
      <style>
        ...
        .buh-card { background:var(--bg2); border:1px solid var(--line); border-radius:6px; padding:16px; }
        .buh-card-label { font-size:11px; color:var(--muted); text-transform:uppercase; letter-spacing:1px; }
        .buh-card-value { font-size:22px; font-weight:900; color:var(--gold); margin-top:6px; white-space:nowrap; }
        ...
        .buh-table td { padding:12px; background:var(--bg2); border:1px solid var(--line); vertical-align:top; }
    ...
    function renderSummary(expenses){
      ...
      $('#buhSummary').innerHTML = `
        <div class="buh-card">
          ...
          <div class="buh-card-value">${moneyShort(total)} ₽</div>
        ...
        ${topCats.map(c => `
          <div class="buh-card">
            ...
            <div class="buh-card-value">${moneyShort(c.sum)} ₽</div>
          </div>
        `).join('')}
      `;
    }
    ...
    function renderTable(expenses){
      ...
      const shown = window.AsgardPagination ? AsgardPagination.paginate(expenses, buhCurrentPage, buhPageSize) : expenses.slice(0, 100);
      ...
      const rows = shown.map(e => {
        ...
        return `
          <tr data-id="${e.id}">
            <td style="white-space:nowrap">${e.date ? AsgardUI.formatDate(e.date) : '—'}</td>
            <td>
      ...
      $('#buhTable').innerHTML = `
        ...
        <div id="buh_pagination" style="margin-top:16px"></div>
      `;
      ...
      if (window.AsgardPagination) {
        const pgEl = document.getElementById('buh_pagination');
        if (pgEl) {
          pgEl.innerHTML = AsgardPagination.renderControls(expenses.length, buhCurrentPage, buhPageSize);
          AsgardPagination.attachHandlers('buh_pagination',
            (p) => { buhCurrentPage = p; renderTable(expenses); },
            (s) => { buhPageSize = s; buhCurrentPage = 1; renderTable(expenses); }
          );
        }
      }
    }
