...
window.AsgardPermitApplications = (function(){
  ...
  const CATEGORIES = {
    safety:    { name: 'Безопасность',    color: 'var(--ok-t)',   icon: '🛡️', order: 1 },
    electric:  { name: 'Электрика',       color: 'var(--amber)',  icon: '⚡', order: 3 },
    special:   { name: 'Спецработы',      color: 'var(--info)',   icon: '🔧', order: 5 },
    medical:   { name: 'Медицина',        color: 'var(--err-t)',  icon: '🏥', order: 2 },
    ...
    transport: { name: 'Транспорт',       color: 'var(--t2)',     icon: '🚛', order: 9 },
    nuclear:   { name: 'Ядерная безопасность', color: 'var(--err-t)', icon: '☢️', order: 8 },
  ...
  async function loadEmployees() {
    ...
    try {
      ...
      allEmployees = (emps || []).filter(e => e.status !== 'fired' && (e.fio||e.full_name||'').trim());
    } catch(e) {
  ...
  async function render({ layout, title }) {
    ...
    async function loadAndRender() {
      ...
      // Event handlers
      const goToForm = () => { location.hash = '#/permit-application-form'; };
      const btnNew = $('#btnNewApp');
      const btnNewEmpty = $('#btnNewAppEmpty');
      if (btnNew) btnNew.onclick = goToForm;
      if (btnNewEmpty) btnNewEmpty.onclick = goToForm;
    ...
    <div class="modal-overlay show" id="empSelectModal">
      <div class="modal-content" style="max-width:800px;max-height:85vh;display:flex;flex-direction:column">
        ...
        <div class="modal-body" style="flex:1;overflow:hidden;display:flex;flex-direction:column;padding:16px 20px">
          ...
          <div class="emp-selector" id="empList" style="flex:1;overflow-y:auto;min-height:0;max-height:none">${renderEmpList('', '')}</div>
        </div>
        <div class="modal-footer" style="display:flex;justify-content:space-between;align-items:center;padding:16px;flex-shrink:0;border-top:1px solid var(--brd)">
  ...
  function openPermitSelectModal(employeeId, currentTypeIds, existingPermits, onConfirm) {
    ...
    // Group types by category, sort by popularity (order field)
    const byCategory = {};
    ...
    // Sort categories by order (popular first)
    const sortedCategories = Object.entries(byCategory).sort((a, b) => {
      const catA = CATEGORIES[a[0]] || { order: 99 };
      const catB = CATEGORIES[b[0]] || { order: 99 };
      return (catA.order || 99) - (catB.order || 99);
    });

    let searchQuery = '';
    let activeCategory = null; // null = show all category cards

    function getPermitIcon(code) {
      const icons = {
    ...
    function renderCategoryCards() {
      return sortedCategories.map(([cat, types]) => {
        const catInfo = CATEGORIES[cat] || { name: cat, color: 'var(--t3)', icon: '📄', order: 99 };
        const selectedInCat = types.filter(t => selected.has(t.id)).length;
        const totalInCat = types.length;

        return `<div class="perm-cat-card" data-cat="${cat}" style="
          background: linear-gradient(135deg, ${catInfo.color}11, ${catInfo.color}05);
          border: 2px solid ${selectedInCat > 0 ? catInfo.color : 'var(--line)'};
        ...
        " onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,0.2)'"
          ...
          <div style="font-size: 32px; margin-bottom: 8px">${catInfo.icon}</div>
          <div style="font-weight: 700; font-size: 15px; margin-bottom: 4px; color: ${catInfo.color}">${catInfo.name}</div>
          <div style="font-size: 12px; color: var(--text-muted)">${totalInCat} разрешени${totalInCat === 1 ? 'е' : totalInCat < 5 ? 'я' : 'й'}</div>
          ${selectedInCat > 0 ? `<div style="position: absolute; top: 8px; right: 8px; background: ${catInfo.color}; color: #fff; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700">${selectedInCat}</div>` : ''}
        </div>`;
      }).join('');
    }

    function renderPermitList(cat) {
      const catInfo = CATEGORIES[cat] || { name: cat, color: 'var(--t3)', icon: '📄' };
      const types = byCategory[cat] || [];
      const filtered = searchQuery
        ? types.filter(t => t.name.toLowerCase().includes(searchQuery.toLowerCase()) || (t.code || '').toLowerCase().includes(searchQuery.toLowerCase()))
        : types;

      if (!filtered.length) return '<div style="text-align:center;padding:40px;color:var(--text-muted)">Нет разрешений по запросу</div>';
      ...
      return filtered.map(t => {
        ...
        const icon = getPermitIcon(t.code);

        let statusBadge = '';
        if (isActive) statusBadge = `<span style="font-size:11px;padding:2px 8px;border-radius:10px;background:rgba(34,197,94,.15);color:var(--ok-t);font-weight:600">${esc(ps.label)}</span>`;
        else if (ps.status === 'expiring') statusBadge = `<span style="font-size:11px;padding:2px 8px;border-radius:10px;background:rgba(245,158,11,.15);color:var(--amber);font-weight:600">${esc(ps.label)}</span>`;
        else if (ps.status === 'expired') statusBadge = `<span style="font-size:11px;padding:2px 8px;border-radius:10px;background:rgba(239,68,68,.15);color:var(--err-t);font-weight:600">${esc(ps.label)}</span>`;

        return `<label class="perm-item ${isChecked ? 'selected' : ''} ${disabled ? 'disabled' : ''}" style="
          display: flex; align-items: center; gap: 12px; padding: 12px 16px;
          border-radius: 10px; cursor: ${disabled ? 'default' : 'pointer'};
          background: ${isChecked ? catInfo.color + '0d' : 'var(--bg-elevated)'};
          border: 1px solid ${isChecked ? catInfo.color + '44' : 'var(--line)'};
          transition: all 0.15s ease; opacity: ${disabled ? '0.5' : '1'};
        " onmouseover="if(!this.classList.contains('disabled'))this.style.borderColor='${catInfo.color}'"
           onmouseout="if(!this.classList.contains('selected'))this.style.borderColor='var(--line)'">
          <input type="checkbox" class="permitCheck" data-id="${t.id}" data-code="${t.code}"
            ${isChecked && !disabled ? 'checked' : ''} ${isActive && !isChecked ? 'checked disabled' : ''} ${disabled ? 'disabled' : ''}
            style="width:18px;height:18px;accent-color:${catInfo.color};flex-shrink:0"/>
          <span style="font-size:22px;flex-shrink:0">${icon}</span>
          <div style="flex:1;min-width:0">
            <div style="font-size:14px;font-weight:600;${disabled ? 'text-decoration:line-through' : ''}">${esc(t.name)}</div>
            ${t.description ? `<div style="font-size:11px;color:var(--text-muted);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(t.description)}</div>` : ''}
          </div>
          ${statusBadge}
        </label>`;
      }).join('');
    ...
    function renderContent() {
      if (activeCategory) {
        const catInfo = CATEGORIES[activeCategory] || { name: activeCategory, icon: '📄', color: 'var(--t3)' };
        return `
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
            <button class="btn ghost" id="btnBackToCats" style="padding:6px 12px;font-size:13px">← Назад</button>
            <span style="font-size:24px">${catInfo.icon}</span>
            <span style="font-size:18px;font-weight:700;color:${catInfo.color}">${catInfo.name}</span>
          </div>
          <div style="display:flex;flex-direction:column;gap:6px" id="permitItemsList">
            ${renderPermitList(activeCategory)}
          </div>`;
      } else {
        const filtered = searchQuery
          ? sortedCategories.filter(([cat, types]) => {
              const catInfo = CATEGORIES[cat] || {};
              return catInfo.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                types.some(t => t.name.toLowerCase().includes(searchQuery.toLowerCase()));
            })
          : sortedCategories;

        if (searchQuery && filtered.length === 0) {
          return '<div style="text-align:center;padding:60px;color:var(--text-muted);font-size:15px">Ничего не найдено по запросу «' + esc(searchQuery) + '»</div>';
        }

        // If searching, show flat list of matching permits
        if (searchQuery) {
          let items = '';
          filtered.forEach(([cat, types]) => {
            const matching = types.filter(t => t.name.toLowerCase().includes(searchQuery.toLowerCase()) || (CATEGORIES[cat]?.name || '').toLowerCase().includes(searchQuery.toLowerCase()));
            matching.forEach(t => {
              const catInfo = CATEGORIES[cat] || { color: 'var(--t3)', icon: '📄' };
              const ps = getPermitStatus(existingPermits, t.code);
              const isActive = ps.status === 'active' && ps.date;
              const isChecked = selected.has(t.id);
              const disabled = isActive && !isChecked;
              const icon = getPermitIcon(t.code);
              items += `<label class="perm-item ${isChecked ? 'selected' : ''}" style="
                display:flex;align-items:center;gap:12px;padding:10px 14px;border-radius:8px;
                cursor:${disabled ? 'default' : 'pointer'};background:var(--bg-elevated);
                border:1px solid ${isChecked ? catInfo.color + '44' : 'var(--line)'};
                opacity:${disabled ? '0.5' : '1'};transition:all .15s">
                <input type="checkbox" class="permitCheck" data-id="${t.id}" data-code="${t.code}" ${isChecked && !disabled ? 'checked' : ''} ${disabled ? 'disabled' : ''} style="width:18px;height:18px;accent-color:${catInfo.color}"/>
                <span style="font-size:20px">${icon}</span>
                <div style="flex:1"><div style="font-weight:600">${esc(t.name)}</div><div style="font-size:11px;color:var(--text-muted)">${catInfo.icon} ${catInfo.name}</div></div>
              </label>`;
            });
          });
        ...
        return `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px" id="permitCatGrid">
          ${renderCategoryCards()}
        </div>`;
      }
    }
    ...
    const presetButtons = Object.entries(PRESETS).map(([key, p]) =>
      `<button class="btn mini ghost" data-preset="${key}" style="border-radius:20px;padding:6px 14px;font-size:12px">${esc(p.name)}</button>`
    ).join('');
    ...
    const copyBtn = otherEmps.length > 0
      ? '<button class="btn mini ghost" id="btnCopyFrom" style="border-radius:20px;padding:6px 14px;font-size:12px">📋 Скопировать с...</button>'
      : '';

    const html = `
    <div class="modal-overlay show" id="permitSelectModal" style="z-index:9999">
      <div class="modal-content" style="max-width:1100px;width:96vw;max-height:92vh;display:flex;flex-direction:column">
        <div class="modal-header" style="padding:16px 24px;flex-shrink:0">
          <div>
            <h3 style="margin:0;font-size:18px">Выбор разрешений</h3>
            <div style="font-size:13px;color:var(--text-muted);margin-top:2px">для <span style="color:var(--primary);font-weight:600">${esc(empName)}</span></div>
          </div>
          <button class="btn ghost btnClose" style="font-size:24px;padding:4px 12px">&times;</button>
        </div>

        <div style="padding:0 24px 12px;flex-shrink:0;display:flex;flex-wrap:wrap;gap:8px;align-items:center;border-bottom:1px solid var(--line)">
          <div style="position:relative;flex:1;min-width:200px;max-width:350px">
            <input type="text" id="permitSearchInput" placeholder="🔍 Поиск разрешений..." style="
              width:100%;padding:10px 16px;border-radius:24px;border:1px solid var(--line);
              background:var(--bg-elevated);color:var(--text);font-size:14px;
            "/>
          </div>
          <div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center">
            <span style="font-size:12px;color:var(--text-muted)">Наборы:</span>
            ${presetButtons}
            ${copyBtn}
          </div>
        </div>

        <div class="modal-body" id="permitModalContent" style="flex:1;overflow-y:auto;padding:20px 24px">
          ${renderContent()}
        </div>

        <div class="modal-footer" style="display:flex;justify-content:space-between;align-items:center;padding:16px 24px;flex-shrink:0;border-top:1px solid var(--line)">
          <div style="display:flex;align-items:center;gap:12px">
            <span style="font-size:14px;font-weight:600" id="permitSelectedCount">✅ Выбрано: ${selected.size}</span>
            ${selected.size > 0 ? `<button class="btn ghost mini" id="btnClearAll" style="font-size:11px;color:var(--text-muted)">Очистить все</button>` : ''}
          </div>
          <div style="display:flex;gap:12px">
            <button class="btn ghost btnClose">Отмена</button>
            <button class="btn primary" id="btnConfirmPermits" style="min-width:140px;font-size:14px;font-weight:700">Применить (${selected.size})</button>
          </div>
        </div>
      </div>
    </div>`;
    ...
    function updatePermitCount() {
      ...
      if (el) el.innerHTML = '✅ Выбрано: ' + selected.size;
      const btn = document.getElementById('btnConfirmPermits');
      if (btn) btn.textContent = 'Применить (' + selected.size + ')';
    }

    function attachHandlers() {
      // Category card clicks
      document.querySelectorAll('#permitSelectModal .perm-cat-card').forEach(card => {
        card.onclick = () => {
      ...
      if (backBtn) backBtn.onclick = () => { activeCategory = null; refreshContent(); };
    ...
    if (btnClear) {
      btnClear.onclick = () => { selected.clear(); refreshContent(); };
    }
  ...
  function openSendConfirmModal(application, onSuccess) {
    const html = `
    <div class="modal-overlay show" id="sendConfirmModal">
  ...
  async function openViewModal(applicationId) {
    ...
    const html = `
    <div class="modal-overlay show" id="viewAppModal">
