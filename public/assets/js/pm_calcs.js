window.AsgardPmCalcsPage = (function(){
  ...
  async function createWorkAssignRequest({tender, actor}){
    ...
    const req = {
      ...
      start_plan: tender.work_start_plan||null,
      end_plan: tender.work_end_plan||null,
      status: "pending",
      requested_at: isoNow(),
  ...
  async function render({layout, title}){
    ...
    async function openTender(tenderId){
      ...
      // 22.05.2026: РП НЕ управляет финальными статусами (Выиграли/Проиграли/Отменён).
      // Это делает ТО/Рук.ТО кнопками в карточке тендера на странице /tenders.
      // РП в этой панели может только: отправить на согласование, видеть текущий статус.
      const _TENDER_TR = {
        'Черновик':              ['Новый'],
        'Новый':                 ['Отправлено на просчёт'],
        'Отправлено на просчёт': ['Согласование ТКП'],
        'Согласование ТКП':      ['ТКП согласовано','Отправлено на просчёт'],
        'ТКП согласовано':       ['Согласование ТКП'],
      ...
      $("#btnSaveStatus").addEventListener("click", async ()=>{
        ...
        // 22.05.2026: автогенерация work УБРАНА.
        // Теперь после «Выиграли» работа создаётся ТОЛЬКО когда Рук.ТО назначит РП
        // через панель «Выигранные тендеры» (POST /api/tenders/:id/assign-work-pm).
        // РП может быть отличаться от того кто считал тендер.
        // См. src/routes/tenders.js → assign-work-pm
