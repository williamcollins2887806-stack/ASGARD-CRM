  ...
  async function openQuickCalcForm(tenderId, tender, est, core, user){
    ...
    const html = `
      <style>
        ...
        .qc-section{border-radius:6px;padding:16px;margin-bottom:16px}
        ...
        .qc-kpi .k{background:var(--bg-card);border-radius:6px;padding:12px;text-align:center}
    ...
    $("#qc_send").addEventListener("click", async ()=>{
      ...
      for(const d of dirs){
        await notify(d.id, "Просчёт на согласование", `${tender.customer_name} — быстрый просчёт`, "#/approvals");
      }
      
      toast("Отправлено", "Просчёт отправлен Ярлу");
      AsgardUI.hideModal();
    });
  ...
  async function render({layout, title}){
      ...
      <div class="panel">
        ...
        <div class="tools">
          ...
          <div class="field" style="min-width:280px">
            ...
            <div style="display:flex; gap:12px; flex-wrap:wrap; padding:10px 12px; border-radius:6px; background:var(--bg-deep)">
    ...
    async function openTender(tenderId){
      ...
      $("#btnSend").addEventListener("click", async ()=>{
        ...
        const dirs = users.filter(u=>isDirRole(u.role) || u.role==="ADMIN");
        for(const d of dirs){
          await notify(d.id, "Просчёт на согласование", `${cur.customer_name} — v${v}`, "#/approvals");
        }
        toast("Согласование","Отправлено Ярлу");
        // refresh list
        tendersAll.splice(tendersAll.findIndex(x=>x.id===tenderId),1,cur);
        tenders = tenders.map(x=>x.id===tenderId?cur:x);
        apply();
