...
window.AsgardGamificationCrud = (function () {
  ...
  async function render({ layout, title }) {
    ...
    layout.innerHTML = `
      <div style="max-width:1300px;margin:0 auto;padding:24px 16px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:8px">
          ...
          <span id="gc-stats" style="font-size:12px;color:var(--t3)">…</span>
        </div>
        <div id="gc-tabs" style="display:flex;gap:4px;margin-bottom:20px;border-bottom:1px solid var(--brd-m);padding-bottom:0"></div>
    ...
    tabs.forEach(tab => {
      ...
      btn.style.cssText = `padding:10px 18px;font-size:13px;font-weight:600;border:none;border-bottom:2px solid transparent;
        background:transparent;cursor:pointer;color:var(--t2);transition:all .2s;`;
      btn.textContent = tab.label;
      btn.addEventListener('click', () => {
        currentTab = tab.id;
        tabBar.querySelectorAll('button').forEach(b => {
          b.style.borderBottomColor = 'transparent'; b.style.color = 'var(--t2)';
        });
        btn.style.borderBottomColor = GOLD; btn.style.color = 'var(--t1)';
        renderTab();
      });
      if (tab.id === currentTab) { btn.style.borderBottomColor = GOLD; btn.style.color = 'var(--t1)'; }
  ...
  async function renderTab() {
    ...
    if (!content) return;
    content.innerHTML = '<div style="text-align:center;padding:40px;color:var(--t3)">Загрузка…</div>';
    try {
  ...
  async function renderShopTab(container) {
    ...
    hdr.innerHTML = `
      <div style="font-size:13px;color:var(--t3)">${items.length} товаров в магазине</div>
    ...
    items.forEach(item => {
      ...
      const pct = item.roulette_pct ? `<span style="color:${GOLD};font-size:11px">🎰 ${item.roulette_pct}% в рулетке</span>` : `<span style="font-size:11px;color:var(--t3)">не в рулетке</span>`;
      ...
      card.innerHTML = `
        <div style="display:flex;align-items:flex-start;gap:12px">
          <div style="font-size:32px;flex-shrink:0;width:48px;height:48px;display:flex;align-items:center;justify-content:center;
            background:var(--bg3);border-radius:12px">${esc(item.icon || '📦')}</div>
          <div style="flex:1;min-width:0">
            <div style="font-size:14px;font-weight:700;color:var(--t1);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(item.name)}</div>
            <div style="font-size:11px;color:var(--t3);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(item.description || '')}</div>
            <div style="display:flex;align-items:center;gap:8px;margin-top:6px;flex-wrap:wrap">
              ...
              <span style="background:var(--bg3);color:var(--t2);border-radius:6px;padding:2px 8px;font-size:11px">${CAT_RU[item.category] || item.category}</span>
            </div>
          </div>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;margin-top:12px;padding-top:10px;border-top:1px solid var(--brd-m)">
          <div style="display:flex;flex-direction:column;gap:2px">
            ${pct}
            <span style="font-size:11px;color:var(--t3)">Запас: ${item.max_stock ? `${item.current_stock}/${item.max_stock}` : '∞'}</span>
          </div>
          <div style="display:flex;gap:6px">
            <button class="gc-edit-shop" data-id="${item.id}" style="padding:6px 12px;border-radius:8px;border:1px solid var(--brd);background:transparent;color:var(--t2);font-size:12px;cursor:pointer">✏ Изменить</button>
      ...
      html: `<div style="padding:16px;display:flex;flex-direction:column;gap:12px;max-height:80vh;overflow-y:auto">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div style="grid-column:1/-1">
            <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Название *</label>
            <input id="gc-name" placeholder="Доширак Ролтон" value="${esc(item?.name || '')}" style="${inputStyle}">
          </div>
          <div style="grid-column:1/-1">
            <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Описание</label>
          ...
          <div>
            <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Цена (руны) *</label>
            <input id="gc-price" type="number" min="1" placeholder="100" value="${item?.price_runes || ''}" style="${inputStyle}">
          </div>
          <div>
            <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Иконка (emoji)</label>
            <input id="gc-icon" placeholder="🍜" value="${esc(item?.icon || '')}" style="${inputStyle}">
          </div>
          <div>
            <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Категория</label>
          ...
          <div>
            <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Редкость</label>
          ...
          <div>
            <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Макс. запас (пусто = ∞)</label>
            <input id="gc-stock" type="number" min="0" placeholder="∞" value="${item?.max_stock || ''}" style="${inputStyle}">
          </div>
          <div>
            <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Текущий остаток</label>
        ...
        <div style="background:rgba(212,168,67,.08);border:1px solid rgba(212,168,67,.2);border-radius:10px;padding:10px;font-size:12px;color:var(--t2)">
  ...
  async function renderPrizesTab(container) {
    ...
    info.style.cssText = 'margin-bottom:16px;padding:12px 16px;background:rgba(74,144,255,.08);border:1px solid rgba(74,144,255,.2);border-radius:12px;font-size:12px;color:var(--t2)';
    info.innerHTML = `🎰 <b>Все призы рулетки — это товары магазина.</b> Чтобы добавить приз — добавьте товар на вкладке "Товары".
      Суммарный вес: <b style="color:var(--t1)">${total_weight}</b>.
    ...
    const table = makeTable(
      ...
      prizes.map(p => {
        ...
        return [
          statusDot,
          `<b>${esc(p.name)}</b><br><span style="font-size:11px;color:var(--t3)">${esc(p.description || '')}</span>`,
          ...
          p.weight,
          p.is_active ? `<b style="color:${GOLD}">${p.roulette_pct}%</b>` : '<span style="color:var(--t3)">0%</span>',
  ...
  async function renderQuestsTab(container) {
    ...
    const table = makeTable(
      ...
      quests.map(q => [
        q.is_active ? '<span style="color:#22c55e">●</span>' : '<span style="color:var(--t3)">●</span>',
        q.quest_type,
        `<b>${esc(q.name)}</b><br><span style="font-size:11px;color:var(--t3)">${esc(q.description || '')}</span>`,
        `${esc(q.target_action)} × ${q.target_count}`,
        `<b style="color:${GOLD}">${q.reward_amount} ${q.reward_type}</b>`,
        `<button class="gc-edit-quest" data-id="${q.id}" style="padding:5px 10px;border-radius:8px;border:1px solid var(--brd);background:transparent;color:var(--t2);font-size:12px;cursor:pointer">✏</button>
  ...
  function showQuestForm(quest) {
    ...
    AsgardUI.showModal({
      ...
      html: `<div style="padding:16px;display:flex;flex-direction:column;gap:12px">
        <div><label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Название *</label>
          <input id="gc-name" placeholder="Выполни 5 объектов за неделю" value="${esc(quest?.name || '')}" style="${inputStyle}"></div>
        <div><label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Описание</label>
          <textarea id="gc-desc" rows="2" style="${inputStyle}resize:vertical">${esc(quest?.description || '')}</textarea></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div><label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Тип</label>
          ...
          <div><label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Иконка</label>
            <input id="gc-icon" placeholder="⚔" value="${esc(quest?.icon || '')}" style="${inputStyle}"></div>
          <div><label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">target_action</label>
            <input id="gc-action" placeholder="complete_work" value="${esc(quest?.target_action || '')}" style="${inputStyle}"></div>
          <div><label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">target_count</label>
            <input id="gc-count" type="number" min="1" value="${quest?.target_count || 1}" style="${inputStyle}"></div>
          <div><label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Награда (кол-во)</label>
            <input id="gc-reward" type="number" min="0" value="${quest?.reward_amount || 0}" style="${inputStyle}"></div>
          <div><label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Тип награды</label>
  ...
  async function renderWinnersTab(container) {
    ...
    filterBar.innerHTML = `
      <div>
        <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">Статус</label>
      ...
      <div>
        <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">С даты</label>
        <input id="gc-w-from" type="date" value="${winnersFilter.from}" style="${inputStyle}width:150px">
      </div>
      <div>
        <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:4px">По дату</label>
      ...
      <button id="gc-w-reset" style="padding:10px 16px;border-radius:10px;border:1px solid var(--brd);background:transparent;color:var(--t2);font-size:13px;cursor:pointer;height:38px">Сбросить</button>`;
  ...
  async function loadWinnersData(container) {
    ...
    if (!tableWrap) return;
    tableWrap.innerHTML = '<div style="text-align:center;padding:20px;color:var(--t3)">Загрузка…</div>';
        ...
        statsEl.innerHTML = `
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px;margin-bottom:16px">
            <div style="background:var(--bg3);border-radius:12px;padding:14px;border:1px solid var(--brd);text-align:center">
              <div style="font-size:24px;font-weight:800;color:var(--t1)">${stats.total_spins}</div>
              <div style="font-size:11px;color:var(--t3);margin-top:4px">Всего спинов</div>
            </div>
            <div style="background:var(--bg3);border-radius:12px;padding:14px;border:1px solid var(--brd);text-align:center">
              <div style="font-size:24px;font-weight:800;color:#f59e0b">${stats.pending_deliveries}</div>
              <div style="font-size:11px;color:var(--t3);margin-top:4px">Ждут выдачи</div>
            </div>
            <div style="background:var(--bg3);border-radius:12px;padding:14px;border:1px solid var(--brd);text-align:center">
              <div style="font-size:24px;font-weight:800;color:#22c55e">${stats.delivered}</div>
              <div style="font-size:11px;color:var(--t3);margin-top:4px">Выдано</div>
            </div>
            <div style="background:var(--bg3);border-radius:12px;padding:14px;border:1px solid var(--brd);text-align:center">
              <div style="font-size:24px;font-weight:800;color:${GOLD}">${stats.rare_wins}</div>
              <div style="font-size:11px;color:var(--t3);margin-top:4px">Редких призов</div>
            </div>
          </div>`;
      }

      if (!wins.length) {
        tableWrap.innerHTML = '<div style="text-align:center;padding:40px;color:var(--t3)">Нет данных по выбранным фильтрам</div>';
        return;
      }
      ...
      const table = makeTable(
        ...
        wins.map(w => {
          ...
          if (w.fulfillment_id && w.delivery_status === 'pending') {
            ...
            actionBtn = `<span style="font-size:11px;color:var(--t3)">${w.delivered_at ? new Date(w.delivered_at).toLocaleDateString('ru') : '✓'}</span>`;
          }

          return [
            `<span style="font-size:12px;white-space:nowrap">${dateStr}</span>`,
            `<b style="font-size:13px">${esc(w.employee_name || '—')}</b><br><span style="font-size:11px;color:var(--t3)">${esc(w.employee_phone || '')}</span>`,
            `<span style="font-size:12px;color:var(--t2)">${esc(w.work_name || '—')}</span>`,
    ...
    t.innerHTML = `
      <thead>
        <tr style="border-bottom:1px solid var(--brd-m)">
          ${headers.map(h => `<th style="text-align:left;padding:10px 12px;color:var(--t3);font-size:11px;font-weight:600;white-space:nowrap">${h}</th>`).join('')}
        </tr>
      </thead>
      <tbody>
        ${rows.map(row => `
          <tr style="border-bottom:1px solid var(--brd-m)" class="gc-row">
            ${row.map(cell => `<td style="padding:10px 12px;color:var(--t1);vertical-align:middle">${cell}</td>`).join('')}
          </tr>`).join('')}
      </tbody>`;
