  ...
  async function render(){
    ...
    try{
      ...
      try{
        ...
        if(app) app.innerHTML = `<div style="padding:18px;color:var(--text-primary,#fff)"><b>Ошибка рендера</b><div style="opacity:.85;margin-top:6px">${(e?.message||String(e)).replace(/</g,"&lt;")}</div><div style="margin-top:10px;display:flex;gap:10px;flex-wrap:wrap"><a href="#/home" style="color:var(--accent,#9bd)">На главную</a><a href="#/diag" style="color:var(--accent,#9bd)">Диагностика</a><button id="btnSafeMode" style="background:var(--bg-elevated,#111827);border:1px solid var(--border,#374151);color:var(--text-primary,#e5e7eb);padding:6px 10px;border-radius:10px;cursor:pointer">Включить safe-mode</button></div></div>`;
        try{
