...
window.AsgardAuth = (function(){
  ...
  function normalizeUserRoles(user){
    ...
    if(primary==="CHIEF_ENGINEER" && !roles.includes("PM")) roles.push("PM");
    if(primary==="DIRECTOR_GEN" && !roles.includes("PM")) roles.push("PM");
  ...
  function canSwitch(user){
    ...
    if(hasPM && roles.includes("CHIEF_ENGINEER")) return {kind:"engineer_pm", a:"CHIEF_ENGINEER", b:"PM"};
    if(hasPM && roles.includes("DIRECTOR_GEN")) return {kind:"director_gen_pm", a:"DIRECTOR_GEN", b:"PM"};
    return null;
  }
