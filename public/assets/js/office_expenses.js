...
window.AsgardOfficeExpensesPage = (function(){
  ...
  const CATEGORIES = [
    ...
    { key: 'project_expense', label: 'Расходы на объект', color: '#f59e0b', icon: '🏗️' },
    { key: 'tools', label: 'Инструменты/СИЗ', color: '#78716c', icon: '🔨' },
  ...
  async function render({layout, title}){
    ...
    let expenses = [];
    let oexpPage = 1;
    let oexpPageSize = window.AsgardPagination ? AsgardPagination.getPageSize() : 20;
    ...
    function renderPage(){
      ...
      const body = `
        <style>
          ...
          .oexp-kpi-card { background:var(--bg3); border:1px solid var(--line); border-radius:6px; padding:16px; text-align:center; }
          .oexp-kpi-label { font-size:11px; color:var(--muted); text-transform:uppercase; }
          .oexp-kpi-value { font-size:22px; font-weight:900; color:var(--gold); margin-top:4px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
          .oexp-kpi-sub { font-size:11px; color:var(--muted); }
          
          .oexp-filters { display:flex; flex-wrap:wrap; gap:12px; margin-bottom:20px; padding:14px; background:var(--bg2); border-radius:6px; align-items:flex-end; }
          ...
          .oexp-cat { background:var(--bg2); border:1px solid var(--line); border-radius:6px; padding:12px; display:flex; align-items:center; gap:10px; }
          ...
          .oexp-table td { padding:10px; background:var(--bg2); border:1px solid var(--line); }
          .oexp-table tr td:first-child { border-radius:6px 0 0 6px; }
          .oexp-table tr td:last-child { border-radius:0 6px 6px 0; }
          .oexp-table tbody tr:nth-child(even) td { background:var(--bg3); }
        ...
        <div class="panel">
          ...
          ${filtered.length ? `
            <table class="oexp-table">
              ...
              <tbody>
                ${(() => { const pageSlice = window.AsgardPagination ? AsgardPagination.paginate(filtered, oexpPage, oexpPageSize) : filtered; return pageSlice.map(e => {
            ...
            <div id="oexp_pagination" style="margin-top:16px"></div>
          ` : `
    ...
    function bindEvents(){
      ...
      if (window.AsgardPagination) {
        const pgEl = document.getElementById('oexp_pagination');
        if (pgEl) {
          const filtered = filterExpenses();
          pgEl.innerHTML = AsgardPagination.renderControls(filtered.length, oexpPage, oexpPageSize);
          AsgardPagination.attachHandlers('oexp_pagination',
            (p) => { oexpPage = p; renderPage(); },
            (s) => { oexpPageSize = s; oexpPage = 1; renderPage(); }
          );
        }
      }
