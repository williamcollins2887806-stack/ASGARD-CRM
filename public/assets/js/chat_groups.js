...
window.AsgardChatGroups = (function(){
  ...
  function isBlankLinkValue(value) {
    if (typeof value !== 'string') return true;
    const trimmed = value.trim();
    return !trimmed || trimmed === '#' || /^(null|undefined)$/i.test(trimmed);
  }

  function getSafeExternalUrl(value) {
    if (isBlankLinkValue(value)) return '';
    const trimmed = value.trim();
    return /^https?:\/\//i.test(trimmed) ? trimmed : '';
  }

  function getSafeInternalAttachmentUrl(value) {
    if (isBlankLinkValue(value)) return '';
    const trimmed = value.trim();

    if (/^\/api\/chat-groups\/\d+\/files\/[^/?#]+$/i.test(trimmed)) return trimmed;
    if (/^\/api\/files\/download\/[^/?#]+$/i.test(trimmed)) return trimmed;

    let storedName = '';
    const uploadMatch = trimmed.match(/^\/uploads\/chat\/([^/?#]+)$/i);
    const relativeMatch = trimmed.match(/^chat\/([^/?#]+)$/i);

    if (uploadMatch) {
      storedName = uploadMatch[1];
    } else if (relativeMatch) {
      storedName = relativeMatch[1];
    } else if (!trimmed.includes('/') && !trimmed.includes('\\')) {
      storedName = trimmed;
    }

    if (!storedName || !currentChatId) return '';
    if (storedName !== storedName.split('/').pop()) return '';
  ...
  function getAttachmentDisplayName(attachment) {
    const rawName = attachment?.file_name || attachment?.original_name || attachment?.filename;
    return typeof rawName === 'string' && rawName.trim() ? rawName.trim() : 'Attachment';
  }

  function getAttachmentHref(attachment) {
    if (!attachment || typeof attachment !== 'object') return '';
    return getSafeInternalAttachmentUrl(attachment.download_url)
      || getSafeInternalAttachmentUrl(attachment.file_url)
      || getSafeInternalAttachmentUrl(attachment.file_path)
      || getSafeInternalAttachmentUrl(attachment.filename);
  }
  ...
  function renderAttachment(attachment) {
    ...
    const baseStyle = 'display:inline-flex;align-items:center;gap:6px;padding:8px 12px;background:rgba(255,255,255,0.06);border-radius:8px;color:var(--gold);text-decoration:none;';

    if (!href) {
      return `<span style="${baseStyle}cursor:default;opacity:0.85;">${icon}${esc(fileName)}</span>`;
    }

    return `<a href="${esc(href)}" download="${esc(fileName)}" style="${baseStyle}">${icon}${esc(fileName)}</a>`;
  }

  function renderAttachments(attachments) {
    if (!Array.isArray(attachments) || attachments.length === 0) return '';
    const items = attachments.map((attachment) => renderAttachment(attachment)).filter(Boolean).join('');
    return items ? `<div class="chat-message-attachments" style="display:flex;flex-wrap:wrap;gap:8px;margin-top:8px;">${items}</div>` : '';
  }
  ...
  async function loadChatMessages(chatId) {
    ...
    mainArea.innerHTML = `
      ...
      ${replyBarHtml}
      <div class="chat-input-area" style="display:flex !important; visibility:visible !important;">
  ...
  function renderMessage(msg, currentUserId, members) {
    ...
    const messageBodyHtml = [
      messageText ? `<div class="chat-message-text">${renderMessageContent(messageText)}</div>` : '',
      renderAttachments(attachments)
    ].filter(Boolean).join('');

    const readHtml = isOwn ? `<span class="chat-msg-read" title="Delivered">&#10003;&#10003;</span>` : '';
    const safeReplyPreview = esc((messageText || '').substring(0, 50).replace(/'/g, "\\'"));
      ...
      <div class="chat-message ${isOwn ? 'own' : ''}" data-msg-id="${msg.id}">
        ...
        <div class="chat-message-bubble">
          ...
          <div class="chat-message-footer">
            <span class="chat-message-time">${time}${msg.edited_at ? ' (edited)' : ''}</span>
            ${readHtml}
          </div>
          ${reactionsHtml}
        </div>
        <div class="chat-msg-actions">
          <button class="chat-msg-action-btn" onclick="AsgardChatGroups.setReply(${msg.id}, '${esc(msg.user_name)}', '${safeReplyPreview}')" title="Reply">&#8617;</button>
          <button class="chat-msg-action-btn" onclick="AsgardChatGroups.showReactionPicker(${msg.id})" title="React">&#9786;</button>
        </div>
      </div>
    `;
  }
  ...
  function renderMessageContent(text) {
    ...
    const imgMatch = text.match(/(?:\u{1F4F7}\s*)?\[[^:\]]+:\s*([^\]]+)\]\(([^)]+)\)/u);
    if (imgMatch) {
      const internalUrl = getSafeInternalAttachmentUrl(imgMatch[2]);
      if (internalUrl) {
        return renderAttachment({ file_name: imgMatch[1], file_path: imgMatch[2], download_url: internalUrl });
      }

      const externalUrl = getSafeExternalUrl(imgMatch[2]);
      if (externalUrl) {
        return `<a href="${esc(externalUrl)}" target="_blank" rel="noopener noreferrer"><img src="${esc(externalUrl)}" alt="${esc(imgMatch[1])}" style="max-width:300px;max-height:300px;border-radius:8px;cursor:pointer;display:block;margin:4px 0"></a>`;
      }

      return esc(imgMatch[1]);
    }

    const fileMatch = text.match(/(?:\u{1F4CE}\s*)?\[([^\]]+)\]\(([^)]+)\)/u);
    if (fileMatch) {
      const internalUrl = getSafeInternalAttachmentUrl(fileMatch[2]);
      if (internalUrl) {
        return renderAttachment({ file_name: fileMatch[1], file_path: fileMatch[2], download_url: internalUrl });
      }

      const externalUrl = getSafeExternalUrl(fileMatch[2]);
      if (externalUrl) {
        return `<a href="${esc(externalUrl)}" target="_blank" rel="noopener noreferrer" download="${esc(fileMatch[1])}" style="display:inline-flex;align-items:center;gap:6px;padding:8px 12px;background:rgba(255,255,255,0.06);border-radius:8px;color:var(--gold);text-decoration:none;">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          ${esc(fileMatch[1])}
        </a>`;
      }

      return esc(fileMatch[1]);
    }
  ...
  async function showNewChatMenu() {
    ...
    const auth = await AsgardAuth.getAuth();
    const myId = auth.user.id;
    const activeUsers = users.filter(u => u.is_active && u.id !== myId && u.name && u.name.trim());
  ...
  async function showMembersModal(chatId) {
    ...
    const availableUsers = users.filter(u => u.is_active && !memberIds.has(u.id) && u.name && u.name.trim());
