  ...
  async function render({layout,title}={}){
    ...
    const html = `
      <div class="panel">
        ...
        <div id="hrrating_pagination"></div>
        </div>
    ...
    if (window.AsgardPagination && rows.length > 0) {
      const tbody = document.getElementById("hrrating_tbody");
      if (tbody) {
        const allRows = Array.from(tbody.querySelectorAll("tr"));
        allRows.forEach((r, i) => {
          if (pageSize > 0) { r.style.display = (i >= (currentPage-1)*pageSize && i < currentPage*pageSize) ? "" : "none"; }
        });
        const pgEl = document.getElementById("hrrating_pagination");
        if (pgEl) {
          pgEl.innerHTML = AsgardPagination.renderControls(allRows.length, currentPage, pageSize);
          AsgardPagination.attachHandlers("hrrating_pagination",
            (p) => { currentPage = p; render(); },
            (s) => { pageSize = s; currentPage = 1; render(); }
          );
        }
      }
    }
