...
window.AsgardPermitsPage = (function(){
  ...
  const PERMIT_TYPES = [
    ...
    { id: 'electro_2', name: 'Электробезопасность, группа II', category: 'electric' },
    { id: 'electro_3', name: 'Электробезопасность, группа III', category: 'electric' },
    { id: 'electro_4', name: 'Электробезопасность, группа IV', category: 'electric' },
    { id: 'electro_5', name: 'Электробезопасность, группа V', category: 'electric' },
  ...
  // Загрузить сотрудников с сервера (вместо IndexedDB)
  async function getEmployeesFromAPI() {
    try {
      const token = getToken();
      const headers = {};
      if (token) headers['Authorization'] = 'Bearer ' + token;
      const resp = await fetch('/api/staff/employees?limit=1000', { headers });
      if (resp.ok) {
        const data = await resp.json();
        return data.employees || [];
      }
    } catch(e) { console.warn('getEmployeesFromAPI fallback:', e.message); }
    // fallback to IndexedDB
    try { return await AsgardDB.getAll('employees') || []; } catch(e) { return []; }
  }
  ...
  async function getWorksFromAPI() {
    try {
      ...
      const headers = {};
      if (token) headers['Authorization'] = 'Bearer ' + token;
      const resp = await fetch('/api/works?limit=500', { headers });
      if (resp.ok) {
        const data = await resp.json();
        return data.works || [];
      }
    } catch(e) { console.warn('getWorksFromAPI fallback:', e.message); }
    try { return await AsgardDB.getAll('works') || []; } catch(e) { return []; }
  }
...
// Получить типы (с сервера или fallback)
  ...
  async function renderEmployeePermits(employeeId, canEdit = false) {
    ...
    let permits = [], currentPage = 1, pageSize = window.AsgardPagination ? AsgardPagination.getPageSize() : 20;
    try {
    ...
    if (permits.length === 0) {
      ...
      const pagedPermits = window.AsgardPagination ? AsgardPagination.paginate(permits, currentPage, pageSize) : permits;
        pagedPermits.forEach(p => {
  ...
  async function openPermitModal(employeeId, permit = null, onSave = null) {
    ...
    const html = `
      <div class="modal-overlay show" id="permitModal">
  ...
  async function openRenewModal(permit, onSave = null) {
    ...
    const html = `
      <div class="modal-overlay show" id="renewModal">
  ...
  async function openBulkRenewModal(permitIds, onSave = null) {
    const html = `
      <div class="modal-overlay show" id="bulkRenewModal">
  ...
  async function renderListTab(container) {
    ...
    // Сотрудники загружаются по требованию (lazy)
    let employees = [];
    ...
    const renderPermitsList = (permits, types, canWrite, canDelete) => {
      ...
      if (window.AsgardPagination) {
        let pgEl = document.getElementById("permits_pagination");
        const listEl = document.getElementById("permitsListContent");
        if (!pgEl && listEl) { pgEl = document.createElement("div"); pgEl.id = "permits_pagination"; listEl.after(pgEl); }
        if (pgEl) {
          pgEl.innerHTML = AsgardPagination.renderControls(permits.length, currentPage, pageSize);
          AsgardPagination.attachHandlers("permits_pagination",
    ...
    if (canWrite && document.getElementById('btnAddNewPermit')) {
      document.getElementById('btnAddNewPermit').onclick = async () => {
        // Выбор сотрудника - загрузка с сервера
        let empList = [];
        try {
          empList = await getEmployeesFromAPI();
        } catch(e) {
          console.warn('Не удалось загрузить сотрудников:', e.message);
        }
        const active = empList.filter(e => e.is_active !== false);

        const selectHtml = `
          <div class="modal-overlay show" id="selectEmployeeModal">
            <div class="modal-content" style="max-width:400px">
              <div class="modal-header">
