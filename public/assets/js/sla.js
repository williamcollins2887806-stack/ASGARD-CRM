...
window.AsgardSLA = (function(){
  function formatUiDate(value, mode){
    if (window.AsgardUI && typeof window.AsgardUI.formatDate === 'function') {
      return window.AsgardUI.formatDate(value, mode);
    }
    if (!value) return '—';
    ...
    return mode === 'yyyy-mm-dd' ? `${y}-${m}-${day}` : `${day}.${m}.${y}`;
  }
  ...
  async function tick(currentUser){
    ...
    const [users, tenders, estimates] = await Promise.all([
      listUsers(),
      AsgardDB.all('tenders').catch(()=>[]),
      AsgardDB.all('estimates').catch(()=>[])
    ]);
    ...
    for(const t of (tenders||[])){
      ...
      for(const uid of recU){
        await notifyOnce({
          ...
          message: `Тендер #${t.id}: осталось ${daysLeft} дн. (до ${formatUiDate(t.docs_deadline)})`,
          link_hash: '#/tenders',
          kind: 'docs_deadline',
          entity_type: 'tender',
          entity_id: t.id,
    ...
    for(const t of (tenders||[])){
      ...
      if(!pm) continue; /* PM check: accept any user assigned as responsible_pm */
      ...
      await notifyOnce({
        ...
        message: `Тендер #${t.id}: нет просчёта. Срок: ${formatUiDate(due)}.`,
        link_hash: '#/pm-calcs',
        kind: 'pm_calc_due',
        entity_type: 'tender',
        entity_id: t.id,
      ...
      if(overdue){
        for(const d of directors){
          await notifyOnce({
            ...
            message: `Тендер #${t.id}: РП ${pm.name||pm.login||pmId} просрочил просчёт (срок был ${formatUiDate(due)}).`,
            link_hash: '#/pm-calcs',
    ...
    for(const e of (estimates||[])){
      ...
      for(const d of directors){
        await notifyOnce({
          ...
          message: `Просчёт #${e.id} (тендер #${e.tender_id}): просрочено согласование. Срок был ${formatUiDate(due)}.`,
          link_hash: '#/approvals',
    ...
    for(const e of (estimates||[])){
      ...
      if(e.pm_id){
        await notifyOnce({
          ...
          message: `Просчёт #${e.id} (тендер #${e.tender_id}): доработка/ответ просрочены. Срок был ${formatUiDate(due)}.`,
          link_hash: '#/pm-calcs',
          kind: 'rework_overdue',
          entity_type: 'estimate',
          entity_id: e.id,
            ...
            if(days===0){
              ...
              for(const uid of recipients){
                await notifyOnce({
                  ...
                  message: `${b.name||b.login||'Сотрудник'}: через ${days} дн. (${formatUiDate(nb)})`,
                  link_hash: '#/birthdays',
                  kind: 'birthday_soon',
                  entity_type: 'user',
                  entity_id: b.id,
            ...
            if(days===0){
              ...
              for(const uid of recipients){
                await notifyOnce({
                  ...
                  message: `${who}: через ${days} дн. (${formatUiDate(nb)})`,
                  link_hash: "#/birthdays?tab=workers",
