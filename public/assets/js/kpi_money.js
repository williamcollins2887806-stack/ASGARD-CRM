...
window.AsgardKpiMoneyPage = (function(){
  ...
  const EXPENSE_CATEGORIES = {
    ...
    chemicals: { label: 'Химия', color: 'var(--amber)' },
    equipment: { label: 'Оборудование', color: 'var(--purple)' },
    logistics: { label: 'Логистика', color: 'var(--err-t)' },
    transfer: { label: 'Трансфер', color: 'var(--cyan)' },
    accommodation: { label: 'Проживание', color: '#14b8a6' },
    subcontract: { label: 'Субподряд', color: '#ec4899' },
    tickets: { label: 'Билеты', color: '#a855f7' },
    daily: { label: 'Суточные', color: '#84cc16' },
    office: { label: 'Офис', color: 'var(--orange)' },
    other: { label: 'Прочее', color: 'var(--t2)' }
  };

  // Маппинг алиасов категорий (русские/старые → стандартные)
  ...
  function normalizeCategory(cat) {
    return CATEGORY_ALIASES[cat] || cat || 'other';
  }
  ...
  async function loadAllIncomes(year, user){
    ...
    for(const inc of incomes){
      const incDate = inc.date || inc.payment_date || inc.created_at;
      if(getYear(incDate) !== year) continue;
      ...
      result.push({
        id: inc.id,
        date: incDate,
        type: inc.type || inc.payment_type || 'other',
        amount: safeNumber(inc.amount),
  ...
  async function render({layout, title}){
    ...
    async function renderChart(){
      ...
      container.querySelectorAll('.chart-bar').forEach(bar => {
        bar.addEventListener('click', () => {
          const month = safeInt(bar.dataset.month);
          if(!month) return;
    ...
    function openAddIncomeModal(){
      ...
      $('#btnSaveIncome')?.addEventListener('click', async () => {
        const amount = safeNumber($('#inc_amount')?.value);
        if(amount <= 0){
