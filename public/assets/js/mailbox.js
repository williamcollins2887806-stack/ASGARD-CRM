...
window.AsgardMailboxPage = (function(){
  ...
  function renderDetail(data) {
    ...
    const t = EMAIL_TYPES[e.email_type] || EMAIL_TYPES.unknown;
    const date = e.email_date ? formatDateTime(e.email_date) : '';
      ...
      aiSectionHtml = `
        <div style="padding:12px 24px;">
          ...
          <details style="margin-bottom:12px;" open>
            ...
            <div style="margin-top:8px; padding:12px; background:var(--bg-elevated); border:1px solid var(--border); border-radius:6px; font-size:13px; line-height:1.6;">${AsgardUI.renderMarkdown(app.ai_report)}</div>
          </details>` : ''}
          ...
          <div style="padding:10px; background:var(--bg-elevated); border-radius:6px; font-size:12px; margin-bottom:12px;">
            <b>Решение:</b> ${esc(app.decision_by_name)} &middot; ${app.decision_at ? formatDateTime(app.decision_at) : ''}
      ...
      ${attachments.length > 0 ? `
        <div style="padding:12px 24px;">
          ...
          <div style="display:flex; flex-wrap:wrap; gap:8px;">
            ${attachments.map(a => `
              <a href="/api/mailbox/attachments/${a.id}/download" download
