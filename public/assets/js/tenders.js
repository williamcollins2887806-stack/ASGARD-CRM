      ...
      btnBulk.addEventListener("click", async ()=>{
        ...
        const html = `
          <div style="margin-bottom:16px">
            ...
            <div style="background:var(--bg-elevated); padding:12px; border-radius:var(--radius-sm); max-height:150px; overflow:auto; font-size:13px">
              ${statusList}
            </div>
          </div>
          <div class="formrow">
...
${docsHtml}</div>
      ...
      if(document.getElementById("btnHistory")){
        document.getElementById("btnHistory").addEventListener("click", async ()=>{
          ...
          showModal("История (audit log)", rows || `<div class="asg-empty"><div class="asg-empty-icon">📭</div><div class="asg-empty-text">Нет данных</div></div>`);
        });
      }
