...
window.AsgardBigScreen = (function(){
  ...
  async function loadData() {
    const [tenders, works, users, employees, cash, permits, permitTypes] = await Promise.all([
      ...
      AsgardDB.getAll('permit_types').catch(()=>[]) || []
    ]);
        ...
        <div>
          ...
          ${expiring.length ? `<table class="bs-tbl">
            ...
            <tbody>${(() => {
              ...
              (d.employees || []).forEach(e => _empMap.set(e.id, e.fio || e.full_name || ''));
              const _ptMap = new Map();
              (d.permitTypes || []).forEach(t => _ptMap.set(t.id, t.name || ''));
              return expiring.map(p => {
                const days = Math.round((new Date(p.expiry_date) - d.now) / 86400000);
                const color = days <= 7 ? 'c-red' : days <= 21 ? 'c-amber' : 'c-green';
                const empName = _empMap.get(p.employee_id) || p.employee_name || p.fio || '\u2014';
                const typeName = _ptMap.get(p.type_id) || p.permit_type || p.category || '';
