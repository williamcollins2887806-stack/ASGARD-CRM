        ...
        for (const w of yearWorks) {
          worksHtml += `
            <div class="site-work-card" data-work-id="${w.id}">
              ...
              ${w.date_start || w.date_end ? `<div class="swc-dates">${w.date_start ? AsgardUI.formatDate(w.date_start) : '?'} — ${w.date_end ? AsgardUI.formatDate(w.date_end) : '?'}</div>` : ''}
            </div>`;
        }
      }
  ...
  async function onMapClickForPlacement(coords) {
    ...
    try {
      const resp = await fetch(
        ...
        { headers: { 'User-Agent': 'AsgardCRM/1.0 (crm@asgard-service.com)' } }
      );
