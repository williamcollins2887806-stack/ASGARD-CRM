window.AsgardDiagPage=(function(){
  const { $, esc, toast, formatDateTime } = AsgardUI;
  ...
  async function buildHtml(){
    ...
    const builtAtDisplay = formatDateTime(builtAt);
    const lastBackupDisplay = formatDateTime(lastBackup);
    ...
    const auditRows = audit.map(a=>`
      <tr>
        <td class="mono">${esc(formatDateTime(a.created_at||""))}</td>
        <td>${esc(a.action||"")}</td>
    ...
    return `
      <div class="diag-grid">
        <div class="card">
          ...
          <div class="kv"><div>Собрано</div><div class="mono">${esc(builtAtDisplay)}</div></div>
        ...
        <div class="card">
          ...
          <div class="kv"><div>Last backup</div><div class="mono">${esc(lastBackupDisplay)}</div></div>
