      ...
      <PullToRefresh onRefresh={fetchData}>
        {!loading && (
          <div className="rounded-2xl p-4 mb-3" style={{ background: 'color-mix(in srgb, var(--bg-surface) 85%, transparent)', backdropFilter: 'blur(8px)', border: '0.5px solid var(--border-norse)', animation: 'fadeInUp var(--motion-normal) var(--ease-spring) forwards' }}>
            ...
            <p className="text-[24px] font-bold" style={{ color: 'var(--text-primary)' }}>{formatMoney(cashBalance)}</p>
            <p className="text-[12px] mt-1" style={{ color: 'var(--text-secondary)' }}>В очереди: {items.length}</p>
          </div>
        )}
        {loading ? <SkeletonList count={4} /> : items.length === 0 ? (
          ...
          <div className="flex flex-col gap-2 pb-4">
            {items.map((item, i) => (
              <button key={item.id || i} onClick={() => { haptic.light(); setDetail(item); }} className="w-full text-left rounded-2xl px-4 py-3 spring-tap" style={{ background: 'color-mix(in srgb, var(--bg-surface) 85%, transparent)', backdropFilter: 'blur(8px)', border: '0.5px solid var(--border-norse)', animation: `fadeInUp var(--motion-normal) var(--ease-spring) ${i * 40}ms both` }}>
                <div className="flex items-start justify-between gap-2">
                  <p className="text-[14px] font-semibold leading-tight" style={{ color: 'var(--text-primary)' }}>{item.title || item.name || `#${item.entity_id || item.id}`}</p>
                  <ChevronRight size={16} style={{ color: 'var(--text-tertiary)', flexShrink: 0, marginTop: 2 }} />
                </div>
                <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                  {item.entity_type && <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold" style={{ background: 'color-mix(in srgb, var(--blue) 15%, transparent)', color: 'var(--blue)' }}>{item.entity_type}</span>}
                  <span className="text-[12px] font-semibold" style={{ color: 'var(--gold)' }}>{formatMoney(item.amount || 0)}</span>
                  {item.initiator_name && <span className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>{item.initiator_name}</span>}
                  {(item.updated_at || item.created_at) && <span className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>{relativeTime(item.updated_at || item.created_at)}</span>}
                </div>
              </button>
            ))}
          </div>
