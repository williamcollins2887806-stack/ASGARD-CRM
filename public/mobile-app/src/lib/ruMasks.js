 ...
 * Адаптация desktop-v2-src/src/lib/ruMasks.js для Field mobile.
 */
...
/** 8XXXXXXXXXX / 9XXXXXXXXX → 7XXXXXXXXXX (11 цифр) или '' */
...
export function phoneError(v) {
  ...
  const d = digitsOf(v);
  return d.length === 10 || d.length === 11 ? null : 'Телефон должен содержать 10-11 цифр';
}

export function snilsError(v) {
  if (v == null || String(v).trim() === '') return null;
  return digitsOf(v).length === 11 ? null : 'СНИЛС — 11 цифр';
}
