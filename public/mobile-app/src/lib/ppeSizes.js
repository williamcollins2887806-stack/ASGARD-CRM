...
/** Options for <select>: include legacy current value if not in catalog. */
export function ppeSizeOptions(fieldKey, current) {
  const list = PPE_SIZE_BY_FIELD[fieldKey] || [];
  const cur = current != null ? String(current).trim() : '';
  const opts = [{ value: '', label: '—' }, ...list.map((v) => ({ value: v, label: v }))];
