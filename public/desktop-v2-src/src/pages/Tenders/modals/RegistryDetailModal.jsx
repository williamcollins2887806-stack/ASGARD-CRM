/**
 * Registry row detail — карточка, история, смета (mockup v3)
 */
...
export default function RegistryDetailModal({ row, onClose, onRefresh }) {
  const { user } = useAuth();
  ...
  const saveComment = () => {
    patchRegistryField(row.id, 'comment_to', comment)
      .then(() => { toast('Сохранено', 'ok'); onRefresh?.(); })
      .catch((e) => toast(e.message, 'err'));
  };

  return (
    <div className="modal-body" style={{ minWidth: 520 }}>
      <h3>Строка реестра #{row.id}</h3>
      <p className="muted">{row.customer_name} — {row.tender_title}</p>
      ...
      {tab === 'info' && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
            ...
            <div><span className="muted">НМЦ</span><br /><strong>{row.tender_price != null ? Number(row.tender_price).toLocaleString('ru-RU') + ' ₽' : '—'}</strong></div>
