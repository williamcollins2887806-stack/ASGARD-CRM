/**
 * Мимир — полировка текста поля доверенности (как в ТКП).
 * Бэк: POST /api/proxies/polish-text + контекст других полей формы.
 */
...
export function ProxyPolishSheet({ text, fieldLabel, context, onApply }) {
  ...
  const run = async (fromText) => {
    ...
    try {
      const res = await polishText({
        text: t,
        field_label: fieldLabel || 'полномочия доверенности',
        context: context || {}
      });
  ...
  return (
    <MCard className="modal-lg">
      <MHead
        icon="✨"
        title="Мимир — переписка текста"
        subtitle={fieldLabel || 'Поле доверенности'}
        accent="gold"
        onClose={close}
      />
      <MBody>
      ...
      <MFoot align="spread">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn
            disabled={busy}
            onClick={() => {
              setSource(polished.trim() || source);
              run(polished.trim() || source);
            }}
          >
            ✨ Ещё раз
          </Btn>
