...
sftp = c.open_sftp()
with sftp.file("/tmp/nmd_ingest3.js", "w") as f:
    f.write(INGEST)
with sftp.file("/tmp/nmd_regen3.js", "w") as f:
    f.write(REGEN)
sftp.close()

_, o, e = c.exec_command("pkill -f nmd_regen || true; pkill -f nmd_ingest || true; echo cleaned", timeout=15)
print(o.read().decode())

print("=== INGEST ===")
_, o, e = c.exec_command("cd /var/www/asgard-crm && node /tmp/nmd_ingest3.js", timeout=900)
print(o.read().decode())
err = e.read().decode()
if err:
...
_, o, e = c.exec_command(
    "cd /var/www/asgard-crm && nohup node /tmp/nmd_regen3.js > /tmp/nmd_regen.log 2>&1 & echo PID:$!",
    timeout=20,
)
print(o.read().decode())
c.close()
