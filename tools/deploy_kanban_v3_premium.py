#!/usr/bin/env python3
"""Deploy Personal Kanban v3 Premium UX v2."""
import io
import os
import sys
...
def main():
    ...
    print("=== Upload kanban v3 premium UX v2 ===")
    for rel in UPLOAD:
        local = ROOT / rel
        if not local.exists():
    ...
    print("\n=== Verify version in index.html ===")
    _, o, _ = c.exec_command(
        f"grep -o \"ASGARD_SHELL_VERSION = '[^']*'\" {PROJECT}/public/index.html | head -1",
        timeout=15,
    )
    print("prod:", o.read().decode().strip())
    c.close()
    print("\nDONE")
